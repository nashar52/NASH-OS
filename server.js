...
// Recruitment is a controlled ATS workflow ledger. It deliberately does not mirror or
// mutate MySQL HR source tables; hired candidates are handed to an authorized onboarding
// process only after the complete human approval chain is recorded.
const recruitment = { requisitions: [], postings: [], candidates: [], interviews: [], scorecards: [], assessments: [], offers: [], backgroundChecks: [], approvals: [], audit: [] };
const RECRUITMENT_LIMIT = 500;
function recruitmentId(prefix) { return `${prefix}-${Date.now()}-${crypto.randomBytes(3).toString('hex')}`; }
function recruitmentAudit(action, actor, detail = {}) { const entry = { id: recruitmentId('REC-AUD'), action, actor, detail, createdAt: new Date().toISOString(), source: 'Recruitment runtime workflow ledger; MySQL remains the HR source of truth' }; recruitment.audit.unshift(entry); if (recruitment.audit.length > RECRUITMENT_LIMIT) recruitment.audit.pop(); return entry; }
function recruitmentCreate(collection, payload, actor) { const entity = { id: recruitmentId(`REC-${collection.slice(0, 3).toUpperCase()}`), status: 'DRAFT', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), ...payload }; recruitment[collection].unshift(entity); recruitmentAudit(`${collection.toUpperCase()}_CREATED`, actor, { id: entity.id }); return entity; }
function recruitmentById(collection, id) { return recruitment[collection].find((item) => item.id === String(id)); }
...
app.post('/api/permissioned-action', async (req, res) => {
  try {
    ...
    const allowed = {
      employee: ['EMPLOYEE_PROFILE_EDIT_REQUEST','SUBMIT_EVIDENCE','EDIT_DRAFT_EVIDENCE','DELETE_DRAFT_EVIDENCE','SELECT_EMPLOYEE','EMPLOYEE_DOCUMENT_UPLOAD','EMPLOYEE_DOCUMENT_REPLACE','EMPLOYEE_DOCUMENT_ARCHIVE','DOCUMENT_DOWNLOAD','CHECK_IN','START_WORKDAY','CHECK_OUT','START_TASK','EMPLOYEE_PERFORMANCE_SELF_REVIEW','EMPLOYEE_SERVICE_REQUEST','SPRINT10_EMPLOYEE_ACTION'],
      manager: ['ADD_TEAM_TASK_REQUEST','EDIT_TEAM_TASK_REQUEST','CANCEL_TEAM_TASK_REQUEST','DELETE_TEAM_TASK_REQUEST','MANAGER_REQUEST_EVIDENCE','MANAGER_ESCALATE_SLA','MANAGER_ACCEPT','MANAGER_RETURN','MANAGER_EVIDENCE_CORRECTION','MANAGER_EVIDENCE_ACCEPT','DOCUMENT_DOWNLOAD','SPRINT10_MANAGER_ACTION'],
    ...
    const receipt = {
      ...
      evidence: { reference: req.body.evidenceReference || `EV-${now.getTime()}`, capturedAt: now.toISOString(), status: 'CAPTURED_RUNTIME_EVIDENCE' },
      auditTrail: { eventId: `AUD-${now.getTime()}`, event: actionType, actor: accessSession.email, recordedAt: now.toISOString(), status: 'RECORDED' },
...
// Sprint 19 Enterprise Organization Core. This is a read-only MySQL adapter plus
// runtime-only change packets; it never creates, alters, or mutates source tables.
const organizationRuntime = { requests: [], receipts: [] };
function orgId(prefix) { return `${prefix}-${Date.now()}-${crypto.randomBytes(3).toString('hex')}`; }
function orgValue(row, keys, fallback = 'Not mapped yet') { const value = valueByCandidates(row, keys); return value === null ? fallback : String(value); }
async function organizationSnapshot() {
  return withConn(async (conn) => {
    const cfg = dbConfig();
    const [departments, positions, employees] = await Promise.all([tableSample(conn, 'departments', 1000), tableSample(conn, 'positions', 1500), tableSample(conn, 'employees', 2000)]);
    const tables = ['legal_entities','branches','business_units','sections','cost_centers','job_families','job_grades'];
    const availability = {};
    for (const table of tables) availability[table] = await tableExists(conn, cfg.database, table);
    const deptRows = departments.map((row, i) => ({ id: orgValue(row, ['id','department_id','department_code','code'], `department-${i + 1}`), name: orgValue(row, ['name','department_name','title','department']), code: orgValue(row, ['department_code','code','id'], '—'), manager: orgValue(row, ['manager_name','manager','head_of_department'], 'Unassigned'), costCenter: orgValue(row, ['cost_center','cost_center_code'], 'Not supported'), source: 'MySQL: departments' }));
    const departmentById = new Map(deptRows.map(x => [x.id, x.name]));
    const employeeRows = employees.map((row, i) => ({ id: orgValue(row, ['id','employee_id','employee_code'], `employee-${i + 1}`), name: employeeDisplayName(row), departmentId: orgValue(row, ['department_id','dept_id'], ''), department: orgValue(row, ['department_name','department','dept_name'], departmentById.get(orgValue(row, ['department_id','dept_id'], '')) || 'Not mapped yet'), positionId: orgValue(row, ['position_id','job_id'], ''), manager: orgValue(row, ['manager_name','line_manager','supervisor_name','manager','manager_id'], 'Unassigned'), source: 'MySQL: employees' }));
    const incumbents = new Map(); employeeRows.forEach(e => { if (e.positionId) incumbents.set(e.positionId, e); });
    const posRows = positions.map((row, i) => { const id = orgValue(row, ['id','position_id','position_code','code'], `position-${i + 1}`); const incumbent = incumbents.get(id); return { id, code: orgValue(row, ['position_code','code','id'], '—'), title: orgValue(row, ['position_name','name','job_title','title']), department: orgValue(row, ['department_name','department'], departmentById.get(orgValue(row, ['department_id','dept_id'], '')) || 'Not mapped yet'), jobFamily: orgValue(row, ['job_family','family'], 'Not supported'), grade: orgValue(row, ['grade','job_grade','level'], 'Not supported'), manager: orgValue(row, ['manager_name','manager','reports_to'], incumbent?.manager || 'Unassigned'), incumbent: incumbent ? incumbent.name : 'Vacant', status: orgValue(row, ['status','position_status'], 'Active source pending'), vacancyState: incumbent ? 'Filled' : 'Vacant', budgetState: orgValue(row, ['budget_status','budgeted'], 'Not supported'), source: 'MySQL: positions' }; });
    const orphanEmployees = employeeRows.filter(e => e.department === 'Not mapped yet' || e.positionId && !posRows.some(p => p.id === e.positionId));
    const missing = Object.entries(availability).filter(([, exists]) => !exists).map(([table]) => table);
    const tree = [{ id: 'company', type: 'company', label: cfg.database || 'Organization', source: 'MySQL schema context', children: deptRows.map(d => ({ id: `department:${d.id}`, type: 'department', label: d.name, source: d.source, children: posRows.filter(p => p.department === d.name).map(p => ({ id: `position:${p.id}`, type: 'position', label: p.title, source: p.source, children: employeeRows.filter(e => e.positionId === p.id).map(e => ({ id: `employee:${e.id}`, type: 'employee', label: e.name, source: e.source, children: [] })) })) })) }];
    return { departments: deptRows, positions: posRows, employees: employeeRows, tree, orphanEmployees, missing, availability };
  });
}
function orgScoped(snapshot, req) { if (req.accessSession.role !== 'manager') return snapshot; const marker = 'manager'; const employees = snapshot.employees.filter(x => x.manager.toLowerCase().includes(marker)); const ids = new Set(employees.map(x => x.positionId)); const positions = snapshot.positions.filter(x => ids.has(x.id) || x.manager.toLowerCase().includes(marker)); const departments = snapshot.departments.filter(x => employees.some(e => e.department === x.name)); const tree = [{ ...snapshot.tree[0], children: departments.map(d => ({ id: `department:${d.id}`, type:'department', label:d.name, source:d.source, children:positions.filter(p=>p.department===d.name).map(p=>({id:`position:${p.id}`,type:'position',label:p.title,source:p.source,children:employees.filter(e=>e.positionId===p.id).map(e=>({id:`employee:${e.id}`,type:'employee',label:e.name,source:e.source,children:[]}))})) })) }]; return { ...snapshot, employees, positions, departments, tree }; }
function organizationReceipt(req, action, entity, impact) { const receipt = { receiptId: orgId('ORG-RCT'), actor: req.accessSession.email, role: req.accessSession.role, timestamp: new Date().toISOString(), source: 'Sprint 19 runtime approval packet; MySQL source records unchanged', affectedEntity: entity, action, approvalStatus: 'PENDING_HUMAN_FINAL_APPROVAL', impact }; organizationRuntime.receipts.unshift(receipt); organizationRuntime.receipts.splice(500); return receipt; }

// Sprint 19 organization APIs. All source reads are SELECT-only through organizationSnapshot.
app.get('/api/organization/summary', requireSession(['manager','hr','executive']), async (req,res) => { const data = orgScoped(await organizationSnapshot(), req); res.json({ metrics: { departments:data.departments.length, positions:data.positions.length, employees:data.employees.length, vacancies:data.positions.filter(x=>x.vacancyState==='Vacant').length, structuralGaps:data.orphanEmployees.length }, companyProfile:{ name: dbConfig().database || 'Organization', source:'MySQL schema context' }, missingPersistentSchema:data.missing, source:'MySQL: departments + positions + employees; runtime packets only' }); });
app.get('/api/organization/tree', requireSession(['manager','hr','executive']), async (req,res) => { const data=orgScoped(await organizationSnapshot(),req); const q=String(req.query.q||'').toLowerCase(); const tree=q ? data.tree.map(root=>({...root,children:root.children.filter(n=>JSON.stringify(n).toLowerCase().includes(q))})) : data.tree; res.json({ tree, source:'MySQL: departments + positions + employees', nodeTypes:['legal_entity','branch','business_unit','department','section','position','employee'], missingPersistentSchema:data.missing }); });
app.get('/api/organization/departments', requireSession(['manager','hr','executive']), async (req,res) => { const data=orgScoped(await organizationSnapshot(),req); res.json({ departments:data.departments, source:'MySQL: departments' }); });
app.get('/api/organization/positions', requireSession(['manager','hr','executive']), async (req,res) => { const data=orgScoped(await organizationSnapshot(),req); res.json({ positions:data.positions, source:'MySQL: positions + employees' }); });
app.get('/api/organization/reporting-structure', requireSession(['manager','hr','executive']), async (req,res) => { const data=orgScoped(await organizationSnapshot(),req); res.json({ reportingLines:data.employees.map(e=>({employee:e.name, manager:e.manager, positionId:e.positionId, source:e.source})), vacantPositions:data.positions.filter(p=>p.vacancyState==='Vacant'), positionsWithoutManagers:data.positions.filter(p=>p.manager==='Unassigned'), orphanRecords:data.orphanEmployees, source:'MySQL: employees + positions' }); });
app.get('/api/organization/approval-hierarchy', requireSession(['manager','hr','executive']), async (req,res) => { const data=orgScoped(await organizationSnapshot(),req); const lines=data.employees.map(e=>({employee:e.name, approver:e.manager, escalation:e.manager==='Unassigned'?'HR final review':'HR final review after manager', status:e.manager==='Unassigned'?'MISSING_APPROVER':'RESOLVED', source:'MySQL: employees manager fields'})); res.json({ lines, missingApprovers:lines.filter(x=>x.status==='MISSING_APPROVER'), humanFinalApprovalRequired:true }); });
app.post('/api/organization/impact-preview', requireSession(['hr','executive']), async (req,res) => { const data=await organizationSnapshot(); const entity=cleanText(req.body?.entity || req.body?.target, 'Affected entity', 160); const matched=data.positions.filter(p=>`${p.id} ${p.title} ${p.department}`.toLowerCase().includes(entity.toLowerCase())); res.json({ affectedEmployees:data.employees.filter(e=>matched.some(p=>p.id===e.positionId)).map(e=>e.name), affectedPositions:matched.map(p=>p.title), affectedReportingLines:matched.length, affectedApprovals:matched.filter(p=>p.manager==='Unassigned').length, payrollOrCostCenterImpact:'Not supported by current detected schema', source:'MySQL read-only preview', saveBlocked:true }); });
app.post('/api/organization/change-request', requireSession(['hr']), async (req,res) => { const action=cleanText(req.body?.action,'Organization action',50).toUpperCase(); if (!['ADD','EDIT','DEACTIVATE','REASSIGN'].includes(action)) return res.status(400).json({error:'Action must be Add, Edit, Deactivate, or Reassign.'}); const entity=cleanText(req.body?.entity,'Affected entity',160); const impact=req.body?.impact || {}; const receipt=organizationReceipt(req,action,entity,impact); const packet={ packetId:orgId('ORG-PKT'), action, entity, status:'PENDING_HUMAN_FINAL_APPROVAL', receiptId:receipt.receiptId, createdAt:receipt.timestamp }; organizationRuntime.requests.unshift(packet); res.status(201).json({ packet, receipt, message:'Organization change request recorded as a runtime approval packet; MySQL was not changed.' }); });
app.get('/api/organization/audit-receipts', requireSession(['hr','executive']), (req,res) => res.json({ receipts:organizationRuntime.receipts, source:'Sprint 19 runtime audit ledger; MySQL unchanged' }));
app.post('/api/organization/change-request/:id/final-decision', requireSession(['executive']), (req,res) => { const packet=organizationRuntime.requests.find(x=>x.packetId===req.params.id); const decision=String(req.body?.decision||'').toUpperCase(); if (!packet) return res.status(404).json({error:'Organization approval packet not found.'}); if (!['APPROVE','REJECT'].includes(decision)) return res.status(400).json({error:'Decision must be APPROVE or REJECT.'}); packet.status=decision==='APPROVE'?'APPROVED_RUNTIME_ONLY':'REJECTED'; const receipt=organizationReceipt(req,`FINAL_${decision}`,packet.entity,{packetId:packet.packetId}); res.json({packet,receipt,message:'Human final decision recorded; no MySQL source record was changed.'}); });


// Sprint 20 — Enterprise Workforce & Position Planning. MySQL reads are SELECT-only;
// requests and audits below are runtime-only packets, never master-data mutations.
const workforceRuntime = { requests: [], receipts: [], audit: [] };
const POSITION_LIFECYCLE = ['DRAFT','REQUESTED','APPROVED','BUDGET_REVIEW','BUDGET_APPROVED','RECRUITING','FILLED','ACTIVE','FROZEN','SUSPENDED','CLOSED','ARCHIVED'];
const HIRING_STATES = ['DRAFT','SUBMITTED','HR_REVIEW','BUDGET_REVIEW','EXECUTIVE_REVIEW','APPROVED','RECRUITMENT_STARTED','FILLED','REJECTED','CLOSED'];
function workforceReceipt(req, actionType, targetType, targetId, status, extra = {}) { const timestamp=new Date().toISOString(); const receipt={ receiptId:orgId('WFP-RCT'), timestamp, actor:req.accessSession.email, actorRole:req.accessSession.role, action:actionType, actionType, targetType, targetId, sourceLabel:'Sprint 20 runtime workflow packet; MySQL source data has not been modified.', status, evidenceReference:orgId('WFP-EV'), auditEventId:orgId('WFP-AUD'), humanAuthorizationStatus: req.accessSession.role==='executive' ? 'RECORDED_HUMAN_EXECUTIVE_DECISION' : 'PENDING_HUMAN_AUTHORIZATION', ...extra }; workforceRuntime.receipts.unshift(receipt); workforceRuntime.audit.unshift({eventId:receipt.auditEventId, receiptId:receipt.receiptId, actionType, targetType,targetId, actor:receipt.actor,actorRole:receipt.actorRole,timestamp,status}); workforceRuntime.receipts.splice(500); workforceRuntime.audit.splice(500); return receipt; }
function workforceMetrics(data) { const total=data.positions.length, filled=data.positions.filter(p=>p.vacancyState==='Filled').length, vacant=data.positions.filter(p=>p.vacancyState==='Vacant').length; const stamp=new Date().toISOString(); const metric=(label,value,basis,limitation=null)=>({label,value,sourceLabel:'MySQL: positions + employees',calculationBasis:basis,timestamp:stamp,limitation}); return { metrics:[metric('Current Headcount',data.employees.length,'COUNT(employees)'),metric('Total Positions',total,'COUNT(positions)'),metric('Filled Positions',filled,'positions with a validated employees.position_id relationship'),metric('Vacant Positions',vacant,'positions without a validated employees.position_id relationship'),metric('Vacancy Rate',total ? `${pct(vacant,total)}%`:'Not Configured','Vacant positions ÷ total positions','Only valid where position-holder relationship exists'),metric('Planned Headcount','Not Configured','No detected persistent planning field','Schema Support Required'),metric('Budgeted Headcount','Not Configured','No detected persistent budget headcount field','Schema Support Required'),metric('Critical Positions','Not Configured','No detected criticality field','Schema Support Required'),metric('Workforce Gap','Not Configured','Planned headcount minus current headcount','Schema Support Required')] }; }
function workforceDashboard(data) { const metrics=workforceMetrics(data).metrics; const byDepartment=[...new Set(data.positions.map(p=>p.department))].map(department=>{const positions=data.positions.filter(p=>p.department===department);const vacant=positions.filter(p=>p.vacancyState==='Vacant').length;return {department,positions:positions.length,vacant,sourceLabel:'MySQL: positions + employees',attention:vacant?'Vacancy review required':'No validated vacancy'};}); return {metrics, departments:byDepartment, workforcePlanning:[{area:'Workforce Demand',value:'Not Configured',sourceLabel:'No persistent demand-plan source',limitation:'Schema Support Required'},{area:'Workforce Supply',value:data.employees.length,sourceLabel:'MySQL: employees',limitation:'Current employee count only'},{area:'Hiring Forecast',value:'Not Configured',sourceLabel:'No persistent forecast source',limitation:'Schema Support Required'},{area:'Turnover Risk',value:'Source Unavailable',sourceLabel:'No detected turnover source',limitation:'AI advisory cannot infer attrition'},{area:'Retirement Risk',value:'Source Unavailable',sourceLabel:'No detected retirement data',limitation:'Schema Support Required'},{area:'Internal Mobility',value:'Source Unavailable',sourceLabel:'No detected mobility history',limitation:'Schema Support Required'},{area:'Promotion Readiness',value:'Source Unavailable',sourceLabel:'No mapped performance/readiness relationship',limitation:'Incomplete data'},{area:'Succession Coverage',value:'Not Configured',sourceLabel:'No successor relationship',limitation:'No identified successors; incomplete data'},{area:'Critical Skill Gaps',value:'Source Unavailable',sourceLabel:'No position skill mapping',limitation:'Schema Support Required'}], policy:{aiAdvisoryOnly:true,noMysqlMutation:true,humanFinalDecisionRequired:true} }; }
function workforceScope(data, req) { if(req.accessSession.role!=='employee') return orgScoped(data,req); const own=data.employees.find(e=>String(e.id)===String(req.accessSession.email)||String(e.name).toLowerCase()===String(req.accessSession.email).toLowerCase()); const positions=own?data.positions.filter(p=>p.id===own.positionId):[]; return {...data,employees:own?[own]:[],positions}; }
app.get('/api/workforce/dashboard', requireSession(['manager','hr','executive']), async (req,res)=>{const data=orgScoped(await organizationSnapshot(),req);res.json({...workforceDashboard(data),sourceLabel:'MySQL read-only workforce adapter'});});
app.get('/api/workforce/positions', requireSession(), async (req,res)=>{const data=workforceScope(await organizationSnapshot(),req);const timestamp=new Date().toISOString();res.json({positions:data.positions.map(p=>({...p,positionId:p.id,positionCode:p.code,positionTitle:p.title,organizationNode:p.department,reportsToPosition:p.manager,currentPositionHolder:p.incumbent,positionStatus:p.status,vacancyStatus:p.vacancyState,budgetStatus:p.budgetState,employeeCount:p.incumbent==='Vacant'?0:1,sourceLabel:p.source,lastVerifiedTimestamp:timestamp})),sourceLabel:'MySQL: positions + employees',separateEntities:true});});
app.get('/api/workforce/vacancies', requireSession(['manager','hr','executive']), async (req,res)=>{const data=orgScoped(await organizationSnapshot(),req);const now=new Date().toISOString();res.json({vacancies:data.positions.filter(p=>p.vacancyState==='Vacant').map(p=>({positionId:p.id,positionTitle:p.title,department:p.department,vacancyStatus:'Vacant',vacancyAge:'Not Configured',vacancyReason:'Not Configured',criticality:'Not Configured',temporaryCoverage:'Not Configured',hiringPriority:'Not Configured',recruitmentStatus:'Not Configured',sourceLabel:'MySQL: positions without validated employee holder',lastVerifiedTimestamp:now})),calculationBasis:'Validated employees.position_id relationship only; not inferred from totals.'});});
app.get('/api/workforce/succession', requireSession(['hr','executive']), async (req,res)=>res.json({items:[],status:'INCOMPLETE_DATA',message:'No source-backed successor, criticality, or readiness relationship is available. No successors were fabricated.',sourceLabel:'MySQL schema inspection',coveragePercentage:'Not Configured'}));
app.post('/api/workforce/impact-preview', requireSession(['hr','executive']), async (req,res)=>{const data=await organizationSnapshot();const target=cleanText(req.body?.target,'Target position',160);const matches=data.positions.filter(p=>`${p.id} ${p.code} ${p.title}`.toLowerCase().includes(target.toLowerCase()));res.json({target,areas:[{area:'Employees',status:'AVAILABLE',impact:data.employees.filter(e=>matches.some(p=>p.id===e.positionId)).length,sourceLabel:'MySQL: employees.position_id'},{area:'Managers',status:'AVAILABLE',impact:matches.filter(p=>p.manager!=='Unassigned').length,sourceLabel:'MySQL: positions manager fields'},{area:'Reporting Relationships',status:'AVAILABLE',impact:matches.length,sourceLabel:'MySQL: employees + positions'},{area:'Open Tasks',status:'UNAVAILABLE',sourceLabel:'Schema relationship not checked'},{area:'Recruitment',status:'UNAVAILABLE',sourceLabel:'No persistent position relationship detected'},{area:'Payroll',status:'UNAVAILABLE',sourceLabel:'No persistent position relationship detected'},{area:'Performance',status:'UNAVAILABLE',sourceLabel:'No persistent position relationship detected'},{area:'Learning',status:'UNAVAILABLE',sourceLabel:'No persistent position relationship detected'},{area:'Government Compliance',status:'UNAVAILABLE',sourceLabel:'No persistent position relationship detected'},{area:'Access Control',status:'UNAVAILABLE',sourceLabel:'No persistent position relationship detected'},{area:'Approval Routes',status:'UNAVAILABLE',sourceLabel:'No persistent position relationship detected'}],readOnly:true,message:'Impact preview is read-only; unavailable areas are not reported as no impact.'});});
app.post('/api/workforce/requests', requireSession(['manager','hr']), async (req,res)=>{const type=String(req.body?.type||'').toUpperCase();if(!['POSITION_LIFECYCLE','HIRING','REPLACEMENT'].includes(type)) return res.status(400).json({error:'Unsupported workforce request type.'});const targetPosition=cleanText(req.body?.targetPosition,'Target position',160);const requestedState=String(req.body?.requestedState||'DRAFT').toUpperCase();if(type==='POSITION_LIFECYCLE'&&!POSITION_LIFECYCLE.includes(requestedState))return res.status(400).json({error:'Unsupported position lifecycle state.'});if(type==='HIRING'&&!HIRING_STATES.includes(requestedState))return res.status(400).json({error:'Unsupported hiring workflow state.'});const replacementType=String(req.body?.replacementType||'').toUpperCase();if(type==='REPLACEMENT'&&!['EMPLOYEE_REPLACEMENT','INTERNAL_TRANSFER','EXTERNAL_HIRING','TEMPORARY_COVERAGE','POSITION_CLOSURE','BUDGET_RELEASE'].includes(replacementType))return res.status(400).json({error:'Unsupported replacement request.'});const status=type==='HIRING'?'DRAFT':'PENDING_HUMAN_AUTHORIZATION';const packet={packetId:orgId('WFP-PKT'),type,targetPosition,requestedState,replacementType:replacementType||null,status,createdAt:new Date().toISOString(),actor:req.accessSession.email};const receipt=workforceReceipt(req,`${type}_REQUEST_CREATED`,'POSITION',targetPosition,status);workforceRuntime.requests.unshift(packet);res.status(201).json({packet,receipt,message:type==='POSITION_LIFECYCLE'?'Position lifecycle request created. MySQL source data has not been modified.':'Workforce request created as a runtime-only packet. MySQL source data has not been modified.'});});
app.post('/api/workforce/requests/:id/decision', requireSession(['executive']), (req,res)=>{const packet=workforceRuntime.requests.find(x=>x.packetId===req.params.id);const decision=String(req.body?.decision||'').toUpperCase();if(!packet)return res.status(404).json({error:'Workforce request not found.'});if(!['APPROVE','REJECT'].includes(decision))return res.status(400).json({error:'Decision must be APPROVE or REJECT.'});packet.status=decision==='APPROVE'?'APPROVED_RUNTIME_ONLY':'REJECTED';const receipt=workforceReceipt(req,`HUMAN_${decision}`,'POSITION',packet.targetPosition,packet.status,{humanAuthorizationStatus:'RECORDED_HUMAN_EXECUTIVE_DECISION'});res.json({packet,receipt,message:'Human final decision recorded as runtime-only. MySQL source data has not been modified.'});});
app.get('/api/workforce/audit', requireSession(['hr','executive']), (req,res)=>res.json({requests:workforceRuntime.requests,receipts:workforceRuntime.receipts,audit:workforceRuntime.audit,sourceLabel:'Sprint 20 runtime audit ledger; MySQL unchanged'}));
...
// Enterprise Recruitment & ATS. Every state-changing endpoint is permissioned to HR,
// creates an audit receipt, and keeps MySQL employee data read-only.
app.get('/api/recruitment/dashboard', requireSession(['hr']), (req, res) => {
  const candidates = recruitment.candidates; const hired = candidates.filter((item) => item.stage === 'HIRED');
  const closed = recruitment.requisitions.filter((item) => item.status === 'CLOSED');
  const avgDays = hired.length ? Math.round(hired.reduce((sum, item) => sum + Math.max(0, (new Date(item.updatedAt) - new Date(item.createdAt)) / 86400000), 0) / hired.length) : 0;
  const spend = recruitment.requisitions.reduce((sum, item) => sum + Number(item.recruitingCost || 0), 0);
  res.json({ records: recruitment, metrics: { openRequisitions: recruitment.requisitions.filter((item) => item.status === 'APPROVED' || item.status === 'POSTED').length, candidates: candidates.length, interviews: recruitment.interviews.length, offersPending: recruitment.offers.filter((item) => item.status === 'PENDING_APPROVAL').length, timeToHireDays: avgDays, costPerHire: hired.length ? Math.round(spend / hired.length) : 0, closedRequisitions: closed.length }, pipeline: ['APPLIED','SCREENING','INTERVIEW','ASSESSMENT','OFFER','HIRED'].map((stage) => ({ stage, count: candidates.filter((item) => item.stage === stage).length })), policy: { mysqlSourceOfTruth: true, databaseSchemaTouched: false, humanApprovalRequired: true, aiAdvisoryOnly: true } });
});
app.post('/api/recruitment/requisitions', requireSession(['hr']), (req, res) => {
  try { const body = req.body || {}; const title = cleanText(body.title, 'Job title', 160); const department = cleanText(body.department, 'Department', 120); const requisition = recruitmentCreate('requisitions', { title, department, location: cleanText(body.location, 'Location', 120, false), employmentType: cleanText(body.employmentType || 'Full-time', 'Employment type', 60), hiringManager: cleanText(body.hiringManager, 'Hiring manager', 120), recruitingCost: Number(body.recruitingCost || 0), approvalStatus: 'PENDING_POSITION_APPROVAL' }, req.accessSession.email); const audit = recruitmentAudit('POSITION_APPROVAL_REQUESTED', req.accessSession.email, { requisitionId: requisition.id }); res.status(201).json({ requisition, audit, message: 'Job requisition created and routed for position approval.' }); } catch (error) { res.status(400).json({ error: error.message }); }
});
app.post('/api/recruitment/requisitions/:id/approval', requireSession(['hr']), (req, res) => {
  const item = recruitmentById('requisitions', req.params.id); const decision = String(req.body?.decision || '').toUpperCase(); if (!item) return res.status(404).json({ error: 'Job requisition not found.' }); if (!['APPROVE','REJECT'].includes(decision)) return res.status(400).json({ error: 'Decision must be APPROVE or REJECT.' }); item.approvalStatus = decision === 'APPROVE' ? 'APPROVED' : 'REJECTED'; item.status = item.approvalStatus; item.updatedAt = new Date().toISOString(); const audit = recruitmentAudit(`POSITION_${item.approvalStatus}`, req.accessSession.email, { requisitionId: item.id, note: String(req.body?.note || '') }); res.json({ requisition: item, audit, message: `Position approval ${item.approvalStatus.toLowerCase()} by authorized reviewer.` });
});
app.post('/api/recruitment/postings', requireSession(['hr']), (req, res) => {
  const requisition = recruitmentById('requisitions', req.body?.requisitionId); if (!requisition) return res.status(400).json({ error: 'Select an existing job requisition.' }); if (requisition.approvalStatus !== 'APPROVED') return res.status(409).json({ error: 'Only approved requisitions may be posted.' }); const channel = String(req.body?.channel || '').toUpperCase(); if (!['INTERNAL','EXTERNAL'].includes(channel)) return res.status(400).json({ error: 'Posting channel must be INTERNAL or EXTERNAL.' }); const posting = recruitmentCreate('postings', { requisitionId: requisition.id, title: requisition.title, channel, careerPortalUrl: `/careers/${requisition.id}`, status: 'POSTED' }, req.accessSession.email); requisition.status = 'POSTED'; requisition.updatedAt = new Date().toISOString(); const audit = recruitmentAudit('JOB_POSTED', req.accessSession.email, { postingId: posting.id, channel }); res.status(201).json({ posting, audit, message: `${channel.toLowerCase()} job posting published to the controlled career portal.` });
});
app.post('/api/recruitment/candidates', requireSession(['hr']), (req, res) => {
  try { const body = req.body || {}; const requisition = recruitmentById('requisitions', body.requisitionId); if (!requisition) return res.status(400).json({ error: 'Select an existing job requisition.' }); const skills = String(body.skills || '').split(',').map((value) => value.trim()).filter(Boolean).slice(0, 30); const candidate = recruitmentCreate('candidates', { requisitionId: requisition.id, fullName: cleanText(body.fullName, 'Candidate name', 160), email: cleanText(body.email, 'Candidate email', 160), skills, resumeText: cleanText(body.resumeText, 'Resume text', 8000, false), resumeParsed: Boolean(String(body.resumeText || '').trim()), matchScore: null, stage: 'APPLIED' }, req.accessSession.email); const audit = recruitmentAudit('CANDIDATE_PROFILE_CREATED', req.accessSession.email, { candidateId: candidate.id, resumeParsed: candidate.resumeParsed }); res.status(201).json({ candidate, audit, message: candidate.resumeParsed ? 'Candidate profile created and resume parsing recorded.' : 'Candidate profile created; attach resume text to enable parsing.' }); } catch (error) { res.status(400).json({ error: error.message }); }
});
app.post('/api/recruitment/candidates/:id/match', requireSession(['hr']), (req, res) => { const candidate = recruitmentById('candidates', req.params.id); if (!candidate) return res.status(404).json({ error: 'Candidate not found.' }); const requisition = recruitmentById('requisitions', candidate.requisitionId); const required = String(req.body?.requiredSkills || '').split(',').map((value) => value.trim().toLowerCase()).filter(Boolean); const matched = candidate.skills.filter((skill) => required.includes(skill.toLowerCase())); candidate.matchScore = required.length ? Math.round((matched.length / required.length) * 100) : 0; candidate.matchExplanation = `Advisory match based on ${matched.length}/${required.length} requested skills.`; candidate.updatedAt = new Date().toISOString(); const audit = recruitmentAudit('AI_MATCH_GENERATED', req.accessSession.email, { candidateId: candidate.id, requisitionId: requisition?.id, score: candidate.matchScore }); res.json({ candidate, audit, boundary: 'AI matching is advisory only; humans control screening and hiring decisions.' }); });
app.post('/api/recruitment/candidates/:id/stage', requireSession(['hr']), (req, res) => { const candidate = recruitmentById('candidates', req.params.id); const stage = String(req.body?.stage || '').toUpperCase(); if (!candidate) return res.status(404).json({ error: 'Candidate not found.' }); if (!['APPLIED','SCREENING','INTERVIEW','ASSESSMENT','OFFER','HIRED','REJECTED'].includes(stage)) return res.status(400).json({ error: 'Invalid candidate stage.' }); candidate.stage = stage; candidate.updatedAt = new Date().toISOString(); const audit = recruitmentAudit('CANDIDATE_STAGE_CHANGED', req.accessSession.email, { candidateId: candidate.id, stage }); res.json({ candidate, audit, message: `Candidate moved to ${stage}.` }); });
app.post('/api/recruitment/interviews', requireSession(['hr']), (req, res) => { const candidate = recruitmentById('candidates', req.body?.candidateId); if (!candidate) return res.status(400).json({ error: 'Select a candidate.' }); const scheduledAt = String(req.body?.scheduledAt || ''); if (Number.isNaN(Date.parse(scheduledAt))) return res.status(400).json({ error: 'A valid interview date and time is required.' }); const interview = recruitmentCreate('interviews', { candidateId: candidate.id, scheduledAt, interviewer: cleanText(req.body?.interviewer, 'Interviewer', 120), format: cleanText(req.body?.format || 'Video', 'Interview format', 50), status: 'SCHEDULED' }, req.accessSession.email); candidate.stage = 'INTERVIEW'; const audit = recruitmentAudit('INTERVIEW_SCHEDULED', req.accessSession.email, { interviewId: interview.id }); res.status(201).json({ interview, audit, message: 'Interview scheduled and candidate pipeline updated.' }); });
app.post('/api/recruitment/scorecards', requireSession(['hr']), (req, res) => { const interview = recruitmentById('interviews', req.body?.interviewId); if (!interview) return res.status(400).json({ error: 'Select an interview.' }); const score = Number(req.body?.score); if (!Number.isFinite(score) || score < 1 || score > 5) return res.status(400).json({ error: 'Score must be between 1 and 5.' }); const scorecard = recruitmentCreate('scorecards', { interviewId: interview.id, score, recommendation: cleanText(req.body?.recommendation, 'Recommendation', 500), status: 'SUBMITTED' }, req.accessSession.email); interview.status = 'COMPLETED'; const audit = recruitmentAudit('INTERVIEW_SCORECARD_SUBMITTED', req.accessSession.email, { scorecardId: scorecard.id }); res.status(201).json({ scorecard, audit, message: 'Interview scorecard submitted for human hiring review.' }); });
app.post('/api/recruitment/assessments', requireSession(['hr']), (req, res) => { const candidate = recruitmentById('candidates', req.body?.candidateId); if (!candidate) return res.status(400).json({ error: 'Select a candidate.' }); const assessment = recruitmentCreate('assessments', { candidateId: candidate.id, name: cleanText(req.body?.name, 'Assessment name', 120), result: cleanText(req.body?.result, 'Assessment result', 500), status: 'COMPLETED' }, req.accessSession.email); candidate.stage = 'ASSESSMENT'; const audit = recruitmentAudit('ASSESSMENT_RECORDED', req.accessSession.email, { assessmentId: assessment.id }); res.status(201).json({ assessment, audit, message: 'Assessment result recorded for human review.' }); });
app.post('/api/recruitment/offers', requireSession(['hr']), (req, res) => { const candidate = recruitmentById('candidates', req.body?.candidateId); if (!candidate) return res.status(400).json({ error: 'Select a candidate.' }); const offer = recruitmentCreate('offers', { candidateId: candidate.id, salary: Number(req.body?.salary || 0), startDate: String(req.body?.startDate || ''), status: 'PENDING_APPROVAL' }, req.accessSession.email); candidate.stage = 'OFFER'; const audit = recruitmentAudit('OFFER_CREATED', req.accessSession.email, { offerId: offer.id }); res.status(201).json({ offer, audit, message: 'Offer created and routed through hiring approval.' }); });
app.post('/api/recruitment/offers/:id/approval', requireSession(['hr']), (req, res) => { const offer = recruitmentById('offers', req.params.id); const decision = String(req.body?.decision || '').toUpperCase(); if (!offer) return res.status(404).json({ error: 'Offer not found.' }); if (!['APPROVE','REJECT'].includes(decision)) return res.status(400).json({ error: 'Decision must be APPROVE or REJECT.' }); offer.status = decision === 'APPROVE' ? 'APPROVED' : 'REJECTED'; offer.updatedAt = new Date().toISOString(); const candidate = recruitmentById('candidates', offer.candidateId); if (decision === 'APPROVE' && candidate) candidate.stage = 'HIRED'; const onboarding = decision === 'APPROVE' ? recruitmentCreate('approvals', { offerId: offer.id, candidateId: candidate?.id, workflow: 'DIGITAL_ONBOARDING_TRIGGERED', status: 'TRIGGERED' }, req.accessSession.email) : null; const audit = recruitmentAudit(`HIRING_${offer.status}`, req.accessSession.email, { offerId: offer.id, onboardingTriggerId: onboarding?.id }); res.json({ offer, onboarding, audit, message: decision === 'APPROVE' ? 'Hiring approved and digital onboarding trigger created.' : 'Offer rejected by authorized reviewer.' }); });
app.post('/api/recruitment/background-checks', requireSession(['hr']), (req, res) => { const candidate = recruitmentById('candidates', req.body?.candidateId); if (!candidate) return res.status(400).json({ error: 'Select a candidate.' }); const check = recruitmentCreate('backgroundChecks', { candidateId: candidate.id, provider: cleanText(req.body?.provider, 'Provider', 120), status: 'REQUESTED' }, req.accessSession.email); const audit = recruitmentAudit('BACKGROUND_CHECK_REQUESTED', req.accessSession.email, { backgroundCheckId: check.id }); res.status(201).json({ check, audit, message: 'Background check workflow requested; external provider action remains authorized and human-controlled.' }); });


// Sprint 21 — Enterprise Workflow Engine. Persisted relational records are the only
// workflow source of truth. This engine is deliberately module-agnostic.
const WORKFLOW_MODULES = new Set(['ORGANIZATION','WORKFORCE_PLANNING','RECRUITMENT','EMPLOYEE_LIFECYCLE','PERFORMANCE','LEARNING','PAYROLL','GOVERNMENT_RELATIONS','DOCUMENTS','REQUESTS']);
const WORKFLOW_STATES = new Set(['DRAFT','SUBMITTED','PENDING','APPROVED','REJECTED','RETURNED','CANCELLED','EXPIRED','ESCALATED','COMPLETED']);
const WORKFLOW_APPROVAL_TYPES = new Set(['SINGLE','SEQUENTIAL','PARALLEL','CONDITIONAL','ROLE','NAMED_USER','EXECUTIVE']);
let workflowSchemaReady = false;
function workflowId(prefix) { return `${prefix}-${Date.now()}-${crypto.randomBytes(4).toString('hex')}`; }
function workflowText(value, field, max = 500, required = true) { return cleanText(value, field, max, required); }
async function ensureWorkflowSchema() {
  if (workflowSchemaReady) return;
  await withConn(async (conn) => {
    // Additive, idempotent migration only: no reset, seed, or JSON columns.
    const tables = [
      `CREATE TABLE IF NOT EXISTS workflow_templates (id VARCHAR(64) PRIMARY KEY, name VARCHAR(160) NOT NULL, module_code VARCHAR(40) NOT NULL, version_no INT NOT NULL, active TINYINT(1) NOT NULL DEFAULT 1, created_by VARCHAR(160) NOT NULL, created_at DATETIME(3) NOT NULL)`,
      `CREATE TABLE IF NOT EXISTS workflow_template_steps (id VARCHAR(64) PRIMARY KEY, template_id VARCHAR(64) NOT NULL, step_order INT NOT NULL, name VARCHAR(160) NOT NULL, approval_type VARCHAR(30) NOT NULL, approver_role VARCHAR(40) NULL, approver_email VARCHAR(160) NULL, condition_key VARCHAR(80) NULL, sla_minutes INT NOT NULL, escalation_role VARCHAR(40) NULL, FOREIGN KEY (template_id) REFERENCES workflow_templates(id), UNIQUE KEY uq_workflow_step_order (template_id, step_order))`,
      `CREATE TABLE IF NOT EXISTS workflow_instances (id VARCHAR(64) PRIMARY KEY, template_id VARCHAR(64) NOT NULL, module_code VARCHAR(40) NOT NULL, subject_type VARCHAR(80) NOT NULL, subject_id VARCHAR(160) NOT NULL, state VARCHAR(30) NOT NULL, created_by VARCHAR(160) NOT NULL, created_role VARCHAR(40) NOT NULL, created_at DATETIME(3) NOT NULL, submitted_at DATETIME(3) NULL, closed_at DATETIME(3) NULL, current_step_order INT NULL, FOREIGN KEY (template_id) REFERENCES workflow_templates(id), KEY ix_workflow_instance_state (state))`,
      `CREATE TABLE IF NOT EXISTS workflow_assignments (id VARCHAR(64) PRIMARY KEY, workflow_id VARCHAR(64) NOT NULL, step_id VARCHAR(64) NOT NULL, assignee_email VARCHAR(160) NULL, assignee_role VARCHAR(40) NULL, assigned_at DATETIME(3) NOT NULL, due_at DATETIME(3) NOT NULL, completed_at DATETIME(3) NULL, status VARCHAR(30) NOT NULL, delegated_from VARCHAR(160) NULL, FOREIGN KEY (workflow_id) REFERENCES workflow_instances(id), FOREIGN KEY (step_id) REFERENCES workflow_template_steps(id), KEY ix_workflow_assignment_open (workflow_id, status))`,
      `CREATE TABLE IF NOT EXISTS workflow_delegations (id VARCHAR(64) PRIMARY KEY, delegator_email VARCHAR(160) NOT NULL, delegate_email VARCHAR(160) NOT NULL, delegation_type VARCHAR(30) NOT NULL, starts_at DATETIME(3) NOT NULL, ends_at DATETIME(3) NULL, active TINYINT(1) NOT NULL, created_by VARCHAR(160) NOT NULL, created_at DATETIME(3) NOT NULL)`,
      `CREATE TABLE IF NOT EXISTS workflow_evidence (id VARCHAR(64) PRIMARY KEY, workflow_id VARCHAR(64) NOT NULL, assignment_id VARCHAR(64) NULL, evidence_label VARCHAR(160) NOT NULL, evidence_reference VARCHAR(500) NOT NULL, submitted_by VARCHAR(160) NOT NULL, submitted_at DATETIME(3) NOT NULL, FOREIGN KEY (workflow_id) REFERENCES workflow_instances(id))`,
      `CREATE TABLE IF NOT EXISTS workflow_receipts (receipt_id VARCHAR(64) PRIMARY KEY, workflow_id VARCHAR(64) NOT NULL, event_id VARCHAR(64) NOT NULL, issued_at DATETIME(3) NOT NULL, actor_email VARCHAR(160) NOT NULL, action_name VARCHAR(50) NOT NULL, source_label VARCHAR(160) NOT NULL, FOREIGN KEY (workflow_id) REFERENCES workflow_instances(id))`,
      `CREATE TABLE IF NOT EXISTS workflow_audit_events (event_id VARCHAR(64) PRIMARY KEY, workflow_id VARCHAR(64) NOT NULL, receipt_id VARCHAR(64) NOT NULL, occurred_at DATETIME(3) NOT NULL, actor_email VARCHAR(160) NOT NULL, actor_role VARCHAR(40) NOT NULL, action_name VARCHAR(50) NOT NULL, decision VARCHAR(30) NULL, evidence_reference VARCHAR(500) NULL, source_label VARCHAR(160) NOT NULL, human_authorized TINYINT(1) NOT NULL, FOREIGN KEY (workflow_id) REFERENCES workflow_instances(id))`,
      `CREATE TABLE IF NOT EXISTS workflow_notifications (id VARCHAR(64) PRIMARY KEY, workflow_id VARCHAR(64) NOT NULL, recipient_email VARCHAR(160) NULL, recipient_role VARCHAR(40) NULL, channel VARCHAR(20) NOT NULL, subject VARCHAR(200) NOT NULL, body VARCHAR(1000) NOT NULL, status VARCHAR(30) NOT NULL, created_at DATETIME(3) NOT NULL, FOREIGN KEY (workflow_id) REFERENCES workflow_instances(id))`
    ];
    for (const sql of tables) await conn.query(sql);
  });
  workflowSchemaReady = true;
}
async function workflowArtifact(conn, workflowIdValue, req, action, decision = null, evidence = null) {
  const now = new Date(); const eventId = workflowId('WFAUD'); const receiptId = workflowId('WFRCT'); const actor = req.accessSession.email; const role = req.accessSession.role;
  await conn.query('INSERT INTO workflow_receipts (receipt_id, workflow_id, event_id, issued_at, actor_email, action_name, source_label) VALUES (?, ?, ?, ?, ?, ?, ?)', [receiptId, workflowIdValue, eventId, now, actor, action, 'Enterprise Workflow Engine']);
  await conn.query('INSERT INTO workflow_audit_events (event_id, workflow_id, receipt_id, occurred_at, actor_email, actor_role, action_name, decision, evidence_reference, source_label, human_authorized) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)', [eventId, workflowIdValue, receiptId, now, actor, role, action, decision, evidence, 'Enterprise Workflow Engine', 1]);
  return { receiptId, workflowId: workflowIdValue, timestamp: now.toISOString(), actor, role, action, decision, evidence, sourceLabel: 'Enterprise Workflow Engine', auditEventId: eventId, humanAuthorization: true };
}
async function routeWorkflowStep(conn, instance, req) {
  const [steps] = await conn.query('SELECT * FROM workflow_template_steps WHERE template_id = ? AND step_order > ? ORDER BY step_order', [instance.template_id, instance.current_step_order || 0]);
  const step = steps.find((candidate) => !candidate.condition_key || String(req.body?.conditionKey || '') === candidate.condition_key);
  if (!step) { await conn.query("UPDATE workflow_instances SET state = 'APPROVED', closed_at = ? WHERE id = ?", [new Date(), instance.id]); return { completed: true }; }
  const now = new Date(); const due = new Date(now.getTime() + step.sla_minutes * 60000);
  const assigneeRole = step.approval_type === 'EXECUTIVE' ? 'executive' : step.approver_role;
  // A parallel step persists one independent assignment per named approver; no encoded JSON lists.
  const configured = step.approval_type === 'PARALLEL' && req.body?.parallelApprovers ? String(req.body.parallelApprovers).split(',').map(x => x.trim()).filter(Boolean) : [step.approver_email];
  const assignmentIds = [];
  for (const originalEmail of configured) {
    let assigneeEmail = originalEmail || null; let delegatedFrom = null;
    if (assigneeEmail) { const [delegates] = await conn.query('SELECT * FROM workflow_delegations WHERE delegator_email=? AND active=1 AND starts_at<=? AND (ends_at IS NULL OR ends_at>=?) ORDER BY created_at DESC LIMIT 1', [assigneeEmail, now, now]); if (delegates[0]) { delegatedFrom=assigneeEmail; assigneeEmail=delegates[0].delegate_email; } }
    const assignmentId = workflowId('WFASN'); assignmentIds.push(assignmentId);
    await conn.query('INSERT INTO workflow_assignments (id, workflow_id, step_id, assignee_email, assignee_role, assigned_at, due_at, status, delegated_from) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [assignmentId, instance.id, step.id, assigneeEmail, assigneeRole, now, due, 'PENDING', delegatedFrom]);
    for (const channel of ['IN_APP','EMAIL_READY','PUSH_READY']) await conn.query('INSERT INTO workflow_notifications (id, workflow_id, recipient_email, recipient_role, channel, subject, body, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [workflowId('WFNTF'), instance.id, assigneeEmail, assigneeRole, channel, `Workflow ${instance.id} requires human decision`, `Step ${step.name} is pending.`, 'QUEUED', now]);
  }
  await conn.query("UPDATE workflow_instances SET state = 'PENDING', current_step_order = ? WHERE id = ?", [step.step_order, instance.id]);
  return { completed: false, assignmentIds, dueAt: due.toISOString(), step };
}
function workflowAssigneeAllowed(assignment, req) { return (assignment.assignee_email && assignment.assignee_email === req.accessSession.email) || (assignment.assignee_role && assignment.assignee_role === req.accessSession.role); }
app.post('/api/workflows/templates', requireSession(['hr','executive']), async (req, res) => {
  try { await ensureWorkflowSchema(); const body = req.body || {}; const module = workflowText(body.moduleCode, 'Module code', 40).toUpperCase(); if (!WORKFLOW_MODULES.has(module)) throw new Error('Unsupported workflow module.'); const steps = Array.isArray(body.steps) ? body.steps : []; if (!steps.length || steps.length > 30) throw new Error('Provide between 1 and 30 workflow steps.'); const id = workflowId('WFT'); await withConn(async (conn) => { await conn.beginTransaction(); try { await conn.query('INSERT INTO workflow_templates (id, name, module_code, version_no, active, created_by, created_at) VALUES (?, ?, ?, ?, 1, ?, ?)', [id, workflowText(body.name, 'Template name', 160), module, Number(body.versionNo || 1), req.accessSession.email, new Date()]); for (let i=0;i<steps.length;i++) { const step=steps[i]; const type=workflowText(step.approvalType, 'Approval type',30).toUpperCase(); if (!WORKFLOW_APPROVAL_TYPES.has(type)) throw new Error('Unsupported approval type.'); await conn.query('INSERT INTO workflow_template_steps (id, template_id, step_order, name, approval_type, approver_role, approver_email, condition_key, sla_minutes, escalation_role) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [workflowId('WFSTP'), id, i+1, workflowText(step.name, 'Step name', 160), type, step.approverRole || null, step.approverEmail || null, step.conditionKey || null, Math.max(1, Math.min(43200, Number(step.slaMinutes || 1440))), step.escalationRole || null]); } await conn.commit(); } catch(e) { await conn.rollback(); throw e; } }); res.status(201).json({ id, status:'ACTIVE', sourceOfTruth:'mysql' }); } catch(e) { res.status(400).json({ error:e.message }); }
});
app.post('/api/workflows', requireSession(), async (req, res) => { try { await ensureWorkflowSchema(); const b=req.body||{}; const id=workflowId('WFI'); await withConn(async conn => { const [t]=await conn.query('SELECT * FROM workflow_templates WHERE id = ? AND active = 1',[workflowText(b.templateId,'Template ID',64)]); if(!t[0]) throw new Error('Active workflow template not found.'); const module=workflowText(b.moduleCode,'Module code',40).toUpperCase(); if(module!==t[0].module_code) throw new Error('Template module does not match workflow module.'); await conn.query('INSERT INTO workflow_instances (id, template_id, module_code, subject_type, subject_id, state, created_by, created_role, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',[id,t[0].id,module,workflowText(b.subjectType,'Subject type',80),workflowText(b.subjectId,'Subject ID',160),'DRAFT',req.accessSession.email,req.accessSession.role,new Date()]); }); res.status(201).json({workflowId:id,state:'DRAFT',sourceOfTruth:'mysql'}); } catch(e){res.status(400).json({error:e.message});} });
app.post('/api/workflows/delegations', requireSession(), async (req,res)=>{ try { await ensureWorkflowSchema(); const b=req.body||{}; const type=workflowText(b.type,'Delegation type',30).toUpperCase(); if(!['TEMPORARY','PERMANENT','OUT_OF_OFFICE'].includes(type)) throw new Error('Unsupported delegation type.'); const id=workflowId('WFDLG'); await withConn(c=>c.query('INSERT INTO workflow_delegations (id, delegator_email, delegate_email, delegation_type, starts_at, ends_at, active, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?)',[id,req.accessSession.email,workflowText(b.delegateEmail,'Delegate email',160),type,new Date(),type==='PERMANENT'?null:new Date(Date.now()+Math.max(1,Number(b.durationMinutes||1440))*60000),req.accessSession.email,new Date()])); res.status(201).json({delegationId:id,status:'ACTIVE'}); }catch(e){res.status(400).json({error:e.message});} });
app.post('/api/workflows/:id/evidence', requireSession(), async (req,res)=>{try{await ensureWorkflowSchema();const b=req.body||{};const id=workflowId('WFEVD');await withConn(c=>c.query('INSERT INTO workflow_evidence (id, workflow_id, assignment_id, evidence_label, evidence_reference, submitted_by, submitted_at) VALUES (?, ?, ?, ?, ?, ?, ?)',[id,req.params.id,b.assignmentId||null,workflowText(b.label,'Evidence label',160),workflowText(b.reference,'Evidence reference',500),req.accessSession.email,new Date()]));res.status(201).json({evidenceId:id});}catch(e){res.status(400).json({error:e.message});}});
app.post('/api/workflows/:id/actions', requireSession(), async (req,res)=>{ try { await ensureWorkflowSchema(); const action=workflowText(req.body?.action,'Action',30).toUpperCase(); if(!['SUBMIT','APPROVE','REJECT','RETURN','CANCEL','RECALL','REASSIGN','ESCALATE'].includes(action)) throw new Error('Unsupported workflow action.'); const result=await withConn(async conn=>{await conn.beginTransaction();try{const [rows]=await conn.query('SELECT * FROM workflow_instances WHERE id = ? FOR UPDATE',[req.params.id]);const instance=rows[0];if(!instance)throw new Error('Workflow not found.');let decision=null, routing=null;if(action==='SUBMIT'){if(instance.created_by!==req.accessSession.email||instance.state!=='DRAFT')throw new Error('Only the creator may submit a draft.');await conn.query("UPDATE workflow_instances SET state='SUBMITTED', submitted_at=? WHERE id=?",[new Date(),instance.id]);routing=await routeWorkflowStep(conn,instance,req);}else if(['CANCEL','RECALL'].includes(action)){if(instance.created_by!==req.accessSession.email)throw new Error('Only the creator may cancel or recall.');await conn.query("UPDATE workflow_instances SET state='CANCELLED', closed_at=? WHERE id=?",[new Date(),instance.id]);decision='CANCELLED';}else{const [assignments]=await conn.query("SELECT * FROM workflow_assignments WHERE workflow_id=? AND status='PENDING' ORDER BY assigned_at FOR UPDATE",[instance.id]);const assignment=assignments.find(x=>workflowAssigneeAllowed(x,req));if(!assignment)throw new Error('No pending assignment is authorized for this human actor.');if(action==='REASSIGN'){if(!req.body?.assigneeEmail)throw new Error('Reassignment requires assigneeEmail.');await conn.query('UPDATE workflow_assignments SET assignee_email=?, assignee_role=NULL WHERE id=?',[workflowText(req.body.assigneeEmail,'Assignee email',160),assignment.id]);}else if(action==='ESCALATE'){await conn.query("UPDATE workflow_instances SET state='ESCALATED' WHERE id=?",[instance.id]);}else{decision=action==='APPROVE'?'APPROVED':action==='REJECT'?'REJECTED':'RETURNED';await conn.query("UPDATE workflow_assignments SET status=?, completed_at=? WHERE id=?",[decision,new Date(),assignment.id]);if(action==='APPROVE'){const [pending]=await conn.query("SELECT id FROM workflow_assignments WHERE workflow_id=? AND status='PENDING'",[instance.id]);if(!pending.length)routing=await routeWorkflowStep(conn,instance,req);}else await conn.query('UPDATE workflow_instances SET state=?, closed_at=? WHERE id=?',[decision,new Date(),instance.id]);}}const artifact=await workflowArtifact(conn,instance.id,req,action,decision,req.body?.evidenceReference||null);await conn.commit();return{artifact,routing};}catch(e){await conn.rollback();throw e;}});res.json({ok:true,...result,sourceOfTruth:'mysql'});}catch(e){res.status(400).json({error:e.message});} });
app.get('/api/workflows/:id', requireSession(), async (req,res)=>{try{await ensureWorkflowSchema();const data=await withConn(async c=>{const [[workflow]] = await c.query('SELECT * FROM workflow_instances WHERE id=?',[req.params.id]);if(!workflow)throw new Error('Workflow not found.');const [assignments]=await c.query('SELECT * FROM workflow_assignments WHERE workflow_id=?',[workflow.id]);const [receipts]=await c.query('SELECT * FROM workflow_receipts WHERE workflow_id=?',[workflow.id]);const [audit]=await c.query('SELECT * FROM workflow_audit_events WHERE workflow_id=?',[workflow.id]);const [notifications]=await c.query('SELECT * FROM workflow_notifications WHERE workflow_id=?',[workflow.id]);const now=Date.now();return{workflow,assignments:assignments.map(a=>({...a,remainingTimeMs:Math.max(0,new Date(a.due_at).getTime()-now),overdue:a.status==='PENDING'&&new Date(a.due_at).getTime()<now})),receipts,audit,notifications};});res.json({...data,sourceOfTruth:'mysql'});}catch(e){res.status(404).json({error:e.message});}});
...
// Sprint 23 — Enterprise Policy & Rules Engine.  These are explicitly runtime-only
// governance packets pending a reviewed MySQL migration; no policy, rule, or source
// HR record is fabricated, seeded, or silently persisted.
const POLICY_CATEGORIES = new Set(['HUMAN_RESOURCES','RECRUITMENT','WORKFORCE_PLANNING','ORGANIZATION','EMPLOYEE_LIFECYCLE','ATTENDANCE','LEAVE','PERFORMANCE','LEARNING','COMPENSATION','PAYROLL','GOVERNMENT_RELATIONS','COMPLIANCE','INFORMATION_SECURITY','DATA_PRIVACY','DELEGATION_OF_AUTHORITY','APPROVAL_MATRIX','DOCUMENT_MANAGEMENT','QUALITY_AND_GOVERNANCE']);
const POLICY_STATES = new Set(['DRAFT','UNDER_REVIEW','LEGAL_REVIEW','COMPLIANCE_REVIEW','HR_REVIEW','EXECUTIVE_REVIEW','APPROVED','PUBLISHED','ACTIVE','SUSPENDED','SUPERSEDED','EXPIRED','ARCHIVED']);
const RULE_TYPES = new Set(['ELIGIBILITY','VALIDATION','APPROVAL','ROUTING','THRESHOLD','DATE','TENURE','GRADE','POSITION','DEPARTMENT','EMPLOYEE_TYPE','COMPENSATION','ATTENDANCE','LEAVE','PERFORMANCE','TRAINING','COMPLIANCE','SAUDI_LABOR','DELEGATION','EXCEPTION','ESCALATION','EVIDENCE_REQUIREMENT']);
const SAFE_POLICY_OPERATORS = new Set(['EQUALS','NOT_EQUALS','GREATER_THAN','GREATER_THAN_OR_EQUAL','LESS_THAN','LESS_THAN_OR_EQUAL','CONTAINS','DOES_NOT_CONTAIN','IN_LIST','NOT_IN_LIST','EXISTS','DOES_NOT_EXIST','BEFORE_DATE','AFTER_DATE','BETWEEN','AND','OR']);
const policyRuntime = { policies:[], rules:[], evaluations:[], exceptions:[], acknowledgements:[], receipts:[], audit:[] };
function policyId(prefix) { return `${prefix}-${Date.now()}-${crypto.randomBytes(4).toString('hex')}`; }
function policyReceipt(req, actionType, targetType, targetId, status, extra={}) { const timestamp=new Date().toISOString(); const receipt={receiptId:policyId('POL-RCT'),timestamp,actor:req.accessSession.email,actorRole:req.accessSession.role,actionType,targetType,targetId,sourceLabel:'Runtime-only; schema support required; MySQL source records unchanged.',status,workflowReference:extra.workflowReference||null,evidenceReference:extra.evidenceReference||null,auditEventId:policyId('POL-AUD'),humanAuthorizationStatus:extra.humanAuthorizationStatus||'PENDING_HUMAN_AUTHORIZATION'}; policyRuntime.receipts.unshift(receipt); policyRuntime.audit.unshift({eventId:receipt.auditEventId,...receipt}); return receipt; }
function policyCondition(condition) { if(!condition || typeof condition!=='object') throw new Error('Each rule condition must be a structured object.'); const operator=cleanText(condition.operator,'Condition operator',50).toUpperCase(); if(!SAFE_POLICY_OPERATORS.has(operator)) throw new Error('Unsupported condition operator.'); if(['AND','OR'].includes(operator)) { if(!Array.isArray(condition.conditions)||!condition.conditions.length) throw new Error(`${operator} requires nested conditions.`); return {operator,conditions:condition.conditions.map(policyCondition)}; } const field=cleanText(condition.field,'Condition field',120); if(!['EXISTS','DOES_NOT_EXIST'].includes(operator) && !Object.hasOwn(condition,'value')) throw new Error('Condition value is required.'); return {field,operator,value:condition.value}; }
function readField(input, field) { return String(field).split('.').reduce((v,k)=>v && typeof v==='object' ? v[k] : undefined,input); }
function matchCondition(c,input) { if(c.operator==='AND') return c.conditions.every(x=>matchCondition(x,input)); if(c.operator==='OR') return c.conditions.some(x=>matchCondition(x,input)); const actual=readField(input,c.field); const value=c.value; const a=typeof actual==='string'?actual.toLowerCase():actual; const b=typeof value==='string'?value.toLowerCase():value; if(c.operator==='EXISTS') return actual!==undefined&&actual!==null&&actual!==''; if(c.operator==='DOES_NOT_EXIST') return actual===undefined||actual===null||actual===''; if(c.operator==='EQUALS') return a===b; if(c.operator==='NOT_EQUALS') return a!==b; if(c.operator==='GREATER_THAN') return Number(actual)>Number(value); if(c.operator==='GREATER_THAN_OR_EQUAL') return Number(actual)>=Number(value); if(c.operator==='LESS_THAN') return Number(actual)<Number(value); if(c.operator==='LESS_THAN_OR_EQUAL') return Number(actual)<=Number(value); if(c.operator==='CONTAINS') return String(actual||'').toLowerCase().includes(String(value).toLowerCase()); if(c.operator==='DOES_NOT_CONTAIN') return !String(actual||'').toLowerCase().includes(String(value).toLowerCase()); if(c.operator==='IN_LIST') return Array.isArray(value)&&value.map(String).includes(String(actual)); if(c.operator==='NOT_IN_LIST') return Array.isArray(value)&&!value.map(String).includes(String(actual)); if(c.operator==='BEFORE_DATE') return Date.parse(actual)<Date.parse(value); if(c.operator==='AFTER_DATE') return Date.parse(actual)>Date.parse(value); if(c.operator==='BETWEEN') return Array.isArray(value)&&value.length===2&&actual>=value[0]&&actual<=value[1]; return false; }
function policyById(id) { return policyRuntime.policies.find(x=>x.policyId===String(id)); }
app.get('/api/policies', requireSession(), (req,res)=> { const published=['PUBLISHED','ACTIVE']; const records=req.accessSession.role==='employee'?policyRuntime.policies.filter(x=>published.includes(x.status)):policyRuntime.policies; res.json({policies:records,sourceLabel:'Runtime-only; schema support required',persistentStorage:false}); });
app.get('/api/policies/:id', requireSession(), (req,res)=> { const policy=policyById(req.params.id); if(!policy)return res.status(404).json({error:'Policy not found.'}); if(req.accessSession.role==='employee'&&!['PUBLISHED','ACTIVE'].includes(policy.status))return res.status(403).json({error:'This policy is not published for employee access.'}); res.json({policy,rules:policyRuntime.rules.filter(x=>x.policyReference===policy.policyId)}); });
app.post('/api/policies', requireSession(['hr']), (req,res)=>{ try {const b=req.body||{}, category=cleanText(b.category,'Policy category',60).toUpperCase(); if(!POLICY_CATEGORIES.has(category))throw new Error('Unsupported policy category.'); const policy={policyId:policyId('POL'),policyCode:cleanText(b.policyCode,'Policy code',60),policyName:cleanText(b.policyName,'Policy name',160),policyCategory:category,policyDescription:cleanText(b.policyDescription,'Policy description',1000),version:cleanText(b.version,'Version',40),effectiveDate:b.effectiveDate?validIsoDate(b.effectiveDate,'Effective date'):null,expiryDate:b.expiryDate?validIsoDate(b.expiryDate,'Expiry date'):null,policyOwner:cleanText(b.policyOwner,'Policy owner',160),approvingAuthority:cleanText(b.approvingAuthority,'Approving authority',160),status:'DRAFT',jurisdiction:cleanText(b.jurisdiction||'Not configured','Jurisdiction',120),scope:cleanText(b.scope||'Not configured','Scope',300),employeePopulation:b.employeePopulation||'Not configured',organizationScope:b.organizationScope||'Not configured',sourceLabel:'Runtime-only; schema support required',lastReviewedTimestamp:null,evidenceReference:b.evidenceReference||null,auditReference:null}; if(policyRuntime.policies.some(x=>x.policyCode===policy.policyCode))return res.status(409).json({error:'Duplicate policy code.'}); policyRuntime.policies.unshift(policy);const receipt=policyReceipt(req,'POLICY_CREATION_REQUEST','POLICY',policy.policyId,'DRAFT',{evidenceReference:policy.evidenceReference});policy.auditReference=receipt.auditEventId;res.status(201).json({policy,receipt});}catch(e){res.status(400).json({error:e.message});} });
app.patch('/api/policies/:id', requireSession(['hr']), (req,res)=>{const policy=policyById(req.params.id);if(!policy)return res.status(404).json({error:'Policy not found.'});if(policy.status!=='DRAFT')return res.status(409).json({error:'Only draft policies can be updated.'});for(const k of ['policyName','policyDescription','policyOwner','approvingAuthority','scope','employeePopulation','organizationScope','evidenceReference'])if(req.body?.[k]!==undefined)policy[k]=cleanText(req.body[k],k,1000);const receipt=policyReceipt(req,'POLICY_UPDATE_REQUEST','POLICY',policy.policyId,'DRAFT',{evidenceReference:policy.evidenceReference});res.json({policy,receipt});});
app.post('/api/policies/:id/transition', requireSession(['hr','executive']), (req,res)=>{const policy=policyById(req.params.id), next=String(req.body?.status||'').toUpperCase();if(!policy)return res.status(404).json({error:'Policy not found.'});if(!POLICY_STATES.has(next))return res.status(400).json({error:'Unsupported policy lifecycle state.'});if(['APPROVED','PUBLISHED','ACTIVE'].includes(next)&&req.accessSession.role!=='executive')return res.status(403).json({error:'Executive human authorization is required.'});const previousStatus=policy.status;policy.status=next;const receipt=policyReceipt(req,'POLICY_LIFECYCLE_TRANSITION','POLICY',policy.policyId,next,{workflowReference:cleanText(req.body?.workflowReference,'Workflow reference',64),evidenceReference:req.body?.evidenceReference||null,humanAuthorizationStatus:req.accessSession.role==='executive'?'RECORDED_HUMAN_EXECUTIVE_DECISION':'PENDING_HUMAN_AUTHORIZATION'});res.json({policy,previousStatus,newStatus:next,receipt});});
app.get('/api/rules', requireSession(['hr','executive']), (req,res)=>res.json({rules:policyRuntime.rules,sourceLabel:'Runtime-only; schema support required'}));
app.get('/api/rules/:id', requireSession(['hr','executive']), (req,res)=>{const rule=policyRuntime.rules.find(x=>x.ruleId===req.params.id);return rule?res.json({rule}):res.status(404).json({error:'Rule not found.'});});
app.post('/api/rules', requireSession(['hr']), (req,res)=>{try{const b=req.body||{},type=cleanText(b.ruleType,'Rule type',60).toUpperCase(),policy=policyById(b.policyReference);if(!RULE_TYPES.has(type))throw new Error('Unsupported rule type.');if(!policy)throw new Error('A valid policy reference is required.');const rule={ruleId:policyId('RUL'),ruleCode:cleanText(b.ruleCode,'Rule code',60),ruleName:cleanText(b.ruleName,'Rule name',160),ruleType:type,policyReference:policy.policyId,module:cleanText(b.module,'Module',80),trigger:cleanText(b.trigger,'Trigger',120),conditions:(b.conditions||[]).map(policyCondition),outcome:cleanText(b.outcome,'Outcome',500),priority:Number(b.priority),effectiveDate:b.effectiveDate?validIsoDate(b.effectiveDate,'Effective date'):null,expiryDate:b.expiryDate?validIsoDate(b.expiryDate,'Expiry date'):null,status:'DRAFT',requiredEvidence:b.requiredEvidence||null,requiredApproval:b.requiredApproval||null,sourceLabel:'Runtime-only; schema support required',version:cleanText(b.version,'Version',40),lastEvaluatedTimestamp:null};if(!Number.isInteger(rule.priority)||rule.priority<1)throw new Error('Priority must be a positive integer.');if(policyRuntime.rules.some(x=>x.ruleCode===rule.ruleCode))return res.status(409).json({error:'Duplicate rule code.'});policyRuntime.rules.unshift(rule);res.status(201).json({rule,receipt:policyReceipt(req,'RULE_CREATION_REQUEST','RULE',rule.ruleId,'DRAFT')});}catch(e){res.status(400).json({error:e.message});}});
app.post('/api/policies/:id/evaluate', requireSession(['hr','executive']), (req,res)=>{const policy=policyById(req.params.id);if(!policy)return res.status(404).json({error:'Policy not found.'});const input=req.body?.input;if(!input||typeof input!=='object'||Array.isArray(input))return res.status(400).json({error:'Structured evaluation input is required.'});const rules=policyRuntime.rules.filter(x=>x.policyReference===policy.policyId&&x.status!=='ARCHIVED');const matchedRules=rules.filter(x=>x.conditions.every(c=>matchCondition(c,input)));const failedRules=rules.filter(x=>!matchedRules.includes(x));const evaluation={evaluationId:policyId('EVAL'),policyId:policy.policyId,ruleIdsEvaluated:rules.map(x=>x.ruleId),module:cleanText(req.body?.module,'Module',80),subjectType:cleanText(req.body?.subjectType,'Subject type',80),subjectId:cleanText(req.body?.subjectId,'Subject ID',160),inputSourceLabels:req.body?.inputSourceLabels||['Source unavailable'],matchedRules,failedRules,warnings:rules.length?'':'No active runtime rules are configured.',requiredApprovals:matchedRules.map(x=>x.requiredApproval).filter(Boolean),requiredEvidence:matchedRules.map(x=>x.requiredEvidence).filter(Boolean),advisoryRecommendation:matchedRules.length?'Rules matched; route any operational decision through Enterprise Workflow Engine.':'No rule matched; human policy review required.',humanDecisionRequired:true,timestamp:new Date().toISOString()};evaluation.receipt=policyReceipt(req,'POLICY_RULE_EVALUATION','POLICY',policy.policyId,'ADVISORY_ONLY');evaluation.auditEventId=evaluation.receipt.auditEventId;policyRuntime.evaluations.unshift(evaluation);rules.forEach(x=>x.lastEvaluatedTimestamp=evaluation.timestamp);res.json({evaluation});});
app.get('/api/policies/conflicts', requireSession(['hr','executive']), (req,res)=>{const conflicts=[];for(const p of policyRuntime.policies){if(['ACTIVE','PUBLISHED'].includes(p.status)&&!p.approvingAuthority)conflicts.push({type:'MISSING_APPROVAL_AUTHORITY',policyId:p.policyId});if(p.jurisdiction!=='Not configured'&&!['SAUDI_ARABIA','SAUDI','GLOBAL'].includes(p.jurisdiction.toUpperCase()))conflicts.push({type:'UNSUPPORTED_JURISDICTION',policyId:p.policyId});}for(const r of policyRuntime.rules){if(!r.requiredEvidence)conflicts.push({type:'MISSING_EVIDENCE_REQUIREMENT',ruleId:r.ruleId});}res.json({conflicts,requiresHumanReview:true,sourceLabel:'Runtime-only conflict register'});});
app.post('/api/policy-exceptions', requireSession(['employee','manager','hr','executive']), (req,res)=>{try{const b=req.body||{},policy=policyById(b.policyReference);if(!policy)throw new Error('A valid policy reference is required.');const exception={exceptionId:policyId('EXC'),type:cleanText(b.type,'Exception type',60).toUpperCase(),policyReference:policy.policyId,ruleReference:b.ruleReference||null,reason:cleanText(b.reason,'Reason',1000),requestedBy:req.accessSession.email,requestedDate:new Date().toISOString(),effectivePeriod:b.effectivePeriod||null,evidence:b.evidence||null,riskAssessment:b.riskAssessment||'Human risk assessment required',approver:null,status:'PENDING_HUMAN_AUTHORIZATION',humanAuthorization:false};const receipt=policyReceipt(req,'POLICY_EXCEPTION_REQUEST','EXCEPTION',exception.exceptionId,exception.status,{evidenceReference:exception.evidence});exception.receipt=receipt.receiptId;exception.auditEvent=receipt.auditEventId;policyRuntime.exceptions.unshift(exception);res.status(201).json({exception,receipt});}catch(e){res.status(400).json({error:e.message});}});
app.post('/api/policy-exceptions/:id/decision', requireSession(['executive']), (req,res)=>{const item=policyRuntime.exceptions.find(x=>x.exceptionId===req.params.id),decision=String(req.body?.decision||'').toUpperCase();if(!item)return res.status(404).json({error:'Exception not found.'});if(!['APPROVE','REJECT'].includes(decision)||!cleanText(req.body?.rationale,'Override rationale',1000))return res.status(400).json({error:'Approve or reject with rationale.'});item.status=decision==='APPROVE'?'APPROVED_HUMAN_EXCEPTION':'REJECTED_HUMAN_EXCEPTION';item.approver=req.accessSession.email;item.humanAuthorization=true;const receipt=policyReceipt(req,'POLICY_EXCEPTION_DECISION','EXCEPTION',item.exceptionId,item.status,{humanAuthorizationStatus:'RECORDED_HUMAN_EXECUTIVE_DECISION'});res.json({exception:item,receipt});});
app.post('/api/policies/:id/acknowledgements', requireSession(), (req,res)=>{const policy=policyById(req.params.id);if(!policy)return res.status(404).json({error:'Policy not found.'});if(req.accessSession.role==='employee'&&!['PUBLISHED','ACTIVE'].includes(policy.status))return res.status(403).json({error:'Only published applicable policies may be acknowledged.'});const ack={employeeId:req.body?.employeeId||req.accessSession.email,policyId:policy.policyId,policyVersion:policy.version,timestamp:new Date().toISOString(),actor:req.accessSession.email,sourceLabel:'Runtime-only; schema support required',evidenceReference:req.body?.evidenceReference||null};ack.receipt=policyReceipt(req,'POLICY_ACKNOWLEDGEMENT','POLICY',policy.policyId,'RECORDED',{evidenceReference:ack.evidenceReference});ack.auditEventId=ack.receipt.auditEventId;policyRuntime.acknowledgements.unshift(ack);res.status(201).json({acknowledgement:ack,receipt:ack.receipt,legalNotice:'Acknowledgement does not equal agreement where a legal distinction applies.'});});
app.get('/api/policy-receipts', requireSession(['hr','executive']), (req,res)=>res.json({receipts:policyRuntime.receipts,audit:policyRuntime.audit}));
app.get('/api/policies/applicability', requireSession(), (req,res)=>res.json({policies:policyRuntime.policies.filter(x=>['PUBLISHED','ACTIVE'].includes(x.status)),sourceLabel:'Runtime-only; employee population matching not configured'}));
...
// This adapter is deliberately advisory-only and runtime-only pending approved MySQL schema support.
// It never sends unrestricted records to a model and never mutates operational source data.
const SPRINT24_ADVISORY_TYPES = new Set(['WORKFORCE_RISK','TURNOVER_RISK','VACANCY_RISK','SUCCESSION_RISK','ORGANIZATION_HEALTH','SPAN_OF_CONTROL_RISK','WORKFORCE_DEMAND_FORECAST','HIRING_PRIORITY','PROMOTION_READINESS','PERFORMANCE_RISK','TRAINING_RECOMMENDATION','SKILL_GAP_DETECTION','RETENTION_RECOMMENDATION','COMPENSATION_REVIEW_SIGNAL','ATTENDANCE_RISK','COMPLIANCE_RISK','POLICY_CONFLICT_RISK','PAYROLL_READINESS_RISK','GOVERNMENT_DOCUMENT_RISK','EMPLOYEE_LIFECYCLE_RISK','WORKFLOW_DELAY_RISK','SLA_BREACH_RISK','DATA_QUALITY_RISK','EVIDENCE_COMPLETENESS_RISK','EXECUTIVE_DECISION_BACKLOG']);
const SPRINT24_DECISIONS = new Set(['ACCEPT_RECOMMENDATION','REJECT_RECOMMENDATION','REQUEST_MORE_EVIDENCE','DEFER_DECISION','ESCALATE_FOR_REVIEW','RECORD_ALTERNATIVE_DECISION','CLOSE_WITHOUT_ACTION']);
const sprint24Runtime = { advisories: [], decisions: [], receipts: [], audit: [] };
function sprint24Id(prefix) { return `${prefix}-${Date.now()}-${crypto.randomBytes(4).toString('hex')}`; }
function sprint24Receipt(req, actionType, advisory) { const timestamp = new Date().toISOString(); const receipt = { receiptId:sprint24Id('AI-RCT'), timestamp, requestId:req.requestId, tenantId:req.accessSession.tenant, tenant:req.accessSession.tenant, actorId:req.accessSession.email, actor:req.accessSession.email, actorRole:req.accessSession.role, actionType, targetType:advisory.subjectType, targetId:advisory.subjectId, sourceLabel:advisory.sourceLabels.join('; '), sourceTimestamp:advisory.sourceTimestamp, status:advisory.humanDecisionStatus, workflowId:advisory.workflowReference, policyEvaluationId:null, evidenceReference:advisory.evidenceReferences, humanAuthorizationStatus:'PENDING_HUMAN_AUTHORIZATION', sourceMutationStatus:'NOT_SUPPORTED', advisoryId:advisory.advisoryId, subjectType:advisory.subjectType, subjectId:advisory.subjectId, sourceLabels:advisory.sourceLabels, confidence:advisory.confidenceLevel, humanReviewRequired:true, workflowReference:advisory.workflowReference, decisionStatus:advisory.humanDecisionStatus }; const audit = { auditEventId:sprint24Id('AI-AUD'), timestamp, tenantId:receipt.tenantId, actorId:receipt.actorId, actor:receipt.actor, actorRole:receipt.actorRole, actionType, targetType:receipt.targetType, targetId:receipt.targetId, requestId:req.requestId, sourceMutationStatus:receipt.sourceMutationStatus }; receipt.auditEventId=audit.auditEventId; sprint24Runtime.receipts.unshift(receipt); sprint24Runtime.audit.unshift(audit); return receipt; }
function sprint24UnavailableAdvisory(req, body) { const advisoryType=cleanText(body.advisoryType,'Advisory type',80).toUpperCase(); if(!SPRINT24_ADVISORY_TYPES.has(advisoryType)) throw new Error('Unsupported advisory type.'); const subjectType=cleanText(body.subjectType,'Subject type',80); const subjectId=cleanText(body.subjectId,'Subject ID',160); const generatedTimestamp=new Date().toISOString(); const advisory={ advisoryId:sprint24Id('ADV'), advisoryType, subjectType, subjectId, tenantId:req.accessSession.tenant, generatedTimestamp, requestedBy:req.accessSession.email, requestingRole:req.accessSession.role, sourceLabels:['Controlled source retrieval: not invoked because AI provider is not configured'], sourceTimestamp:generatedTimestamp, dataCoverage:'Source retrieval not executed', dataLimitations:['AI provider not configured.','No recommendation was generated.','No persistent Sprint 24 advisory storage schema is configured.'], confidenceLevel:'Insufficient Data', riskLevel:'Unable to Assess', riskCalculationBasis:'No model or controlled source summary was available for evaluation.', recommendation:'Advisory unavailable — configure an approved provider and controlled source mappings, then request human review.', explanation:'No AI analysis was performed. This response is a controlled unavailable state, not a risk finding.', keyFactors:[], contradictingFactors:[], missingData:['Approved AI provider configuration','Controlled source mapping for requested advisory'], policyReferences:[], ruleEvaluationReferences:[], workflowReference:null, evidenceReferences:[], humanReviewRequired:true, humanDecisionStatus:'PENDING_HUMAN_REVIEW', aiAdvisoryOnly:true, notFinalDecision:true, alternativeInterpretations:['The subject may require normal operational review; no AI conclusion can be drawn.'], whatWasNotEvaluated:['No model invocation','No unrestricted database records','No protected attributes'] }; advisory.receipt=sprint24Receipt(req,'AI_ADVISORY_UNAVAILABLE',advisory); advisory.receiptId=advisory.receipt.receiptId; advisory.auditEventId=advisory.receipt.auditEventId; return advisory; }
function sprint24CanView(req, advisory) { if(advisory.tenantId!==req.accessSession.tenant) return false; if(req.accessSession.role==='employee') return advisory.subjectType==='EMPLOYEE' && advisory.subjectId===req.accessSession.email && ['TRAINING_RECOMMENDATION','SKILL_GAP_DETECTION'].includes(advisory.advisoryType); return ['manager','hr','executive'].includes(req.accessSession.role); }
app.post('/api/ai/advisories', requireSession(['hr','executive','manager','employee']), (req,res)=>{ try { const advisory=sprint24UnavailableAdvisory(req,req.body||{}); if(req.accessSession.role==='employee' && !(advisory.subjectType==='EMPLOYEE' && advisory.subjectId===req.accessSession.email && ['TRAINING_RECOMMENDATION','SKILL_GAP_DETECTION'].includes(advisory.advisoryType))) return res.status(403).json({error:'Employees may request only their authorized development advisories.'}); sprint24Runtime.advisories.unshift(advisory); res.status(503).json({advisory, receipt:advisory.receipt, status:'AI_PROVIDER_NOT_CONFIGURED', message:'AI provider not configured. No advisory was generated.', humanReviewRequired:true}); } catch(e) { res.status(400).json({error:e.message}); } });
app.get('/api/ai/advisories', requireSession(), (req,res)=>res.json({advisories:sprint24Runtime.advisories.filter(x=>sprint24CanView(req,x)).slice(0,100), persistence:'Runtime-only; schema support required', providerStatus:'AI_PROVIDER_NOT_CONFIGURED'}));
app.get('/api/ai/provider-health', requireSession(['hr','executive']), (req,res) => { const configured = Boolean(process.env.NASH_AI_PROVIDER && process.env.NASH_AI_MODEL && process.env.NASH_AI_CREDENTIALS_CONFIGURED === 'true'); res.status(configured ? 200 : 503).json({ providerConfigured: configured, status: configured ? 'ADVISORY_PROVIDER_READY' : 'AI_PROVIDER_NOT_CONFIGURED', credentialsStatus: configured ? 'CONFIGURED' : 'MISSING_OR_UNCONFIRMED', modelStatus: configured ? 'CONFIGURED' : 'NOT_CONFIGURED', sourceMappingStatus: 'NOT_CONFIGURED', outputValidationRequired: true, providerName: configured ? 'configured' : null, secretsExposed: false, advisoryExecutionEnabled: false }); });
app.get('/api/ai/advisories/:id', requireSession(), (req,res)=>{ const advisory=sprint24Runtime.advisories.find(x=>x.advisoryId===req.params.id); if(!advisory)return res.status(404).json({error:'Advisory not found.'}); if(!sprint24CanView(req,advisory))return res.status(404).json({error:'Advisory not found.'}); res.json({advisory}); });
app.get('/api/ai/advisories/:id/explainability', requireSession(), (req,res)=>{ const advisory=sprint24Runtime.advisories.find(x=>x.advisoryId===req.params.id); if(!advisory||!sprint24CanView(req,advisory))return res.status(404).json({error:'Advisory not found.'}); res.json({advisoryId:advisory.advisoryId, sourceLabels:advisory.sourceLabels, explanation:advisory.explanation, keyFactors:advisory.keyFactors, contradictingFactors:advisory.contradictingFactors, missingData:advisory.missingData, limitations:advisory.dataLimitations, whatWasNotEvaluated:advisory.whatWasNotEvaluated, alternativeInterpretations:advisory.alternativeInterpretations}); });
app.get('/api/ai/advisories/:id/evidence', requireSession(), (req,res)=>{const advisory=sprint24Runtime.advisories.find(x=>x.advisoryId===req.params.id);if(!advisory||!sprint24CanView(req,advisory))return res.status(404).json({error:'Advisory not found.'});res.json({advisoryId:advisory.advisoryId,evidenceReferences:advisory.evidenceReferences,sourceLabels:advisory.sourceLabels,limitations:advisory.dataLimitations});});
app.get('/api/ai/executive-risk-summary', requireSession(['executive']), (req,res)=>res.json({tenant:req.accessSession.tenant, criticalAdvisories:[], unresolvedHumanDecisions:sprint24Runtime.advisories.filter(x=>x.tenantId===req.accessSession.tenant&&x.humanDecisionStatus==='PENDING_HUMAN_REVIEW').length, lowConfidenceRecommendations:[], dataQualityConcerns:['AI provider not configured'], sourceLabels:['Sprint 24 runtime advisory ledger'], limitations:['No source-backed advisory may be generated until an approved provider and controlled source mappings are configured.'], humanReviewRequired:true}));
app.get('/api/ai/workforce-intelligence', requireSession(['manager','hr','executive']), (req,res)=>res.status(503).json({status:'AI_PROVIDER_NOT_CONFIGURED',sourceLabels:['Sprint 20 Workforce & Position Planning integration boundary'],limitations:['Planning data not configured for AI retrieval.'],riskLevel:'Unable to Assess',humanReviewRequired:true}));
app.get('/api/ai/employee-development', requireSession(), (req,res)=>res.status(503).json({status:'AI_PROVIDER_NOT_CONFIGURED',sourceLabels:['Learning and employee lifecycle integration boundary'],limitations:['No configured provider or authorized development evidence mapping.'],humanReviewRequired:true}));
app.get('/api/ai/policy-compliance', requireSession(['hr','executive']), (req,res)=>res.status(503).json({status:'AI_PROVIDER_NOT_CONFIGURED',sourceLabels:['Sprint 23 Policy & Rules Engine integration boundary'],limitations:['Legal and compliance review required; no AI conclusion generated.'],humanReviewRequired:true}));
app.get('/api/ai/workflow-delays', requireSession(['manager','hr','executive']), (req,res)=>res.status(503).json({status:'AI_PROVIDER_NOT_CONFIGURED',sourceLabels:['Sprint 21 Enterprise Workflow Engine integration boundary'],limitations:['No workflow-delay advisory generated.'],humanReviewRequired:true}));
app.post('/api/ai/advisories/:id/decisions', requireSession(['manager','hr','executive']), (req,res)=>{try{const advisory=sprint24Runtime.advisories.find(x=>x.advisoryId===req.params.id);if(!advisory||!sprint24CanView(req,advisory))return res.status(404).json({error:'Advisory not found.'});const decision=cleanText(req.body?.decision,'Decision',60).toUpperCase();if(!SPRINT24_DECISIONS.has(decision))throw new Error('Unsupported human decision.');const rationale=cleanText(req.body?.rationale,'Rationale',1000);if(['ACCEPT_RECOMMENDATION','REJECT_RECOMMENDATION','RECORD_ALTERNATIVE_DECISION'].includes(decision)&&req.accessSession.role==='manager')return res.status(403).json({error:'Manager review cannot record a final advisory decision.'});const record={decisionId:sprint24Id('AI-DEC'),advisoryId:advisory.advisoryId,actor:req.accessSession.email,actorRole:req.accessSession.role,decision,rationale,evidence:cleanText(req.body?.evidence,'Evidence reference',500,false)||null,timestamp:new Date().toISOString(),workflowReference:cleanText(req.body?.workflowReference,'Workflow reference',64,false)||null,policyReference:cleanText(req.body?.policyReference,'Policy reference',64,false)||null};advisory.humanDecisionStatus=decision;record.receipt=sprint24Receipt(req,'AI_HUMAN_DECISION_RECORDED',advisory);record.receiptId=record.receipt.receiptId;record.auditEventId=record.receipt.auditEventId;sprint24Runtime.decisions.unshift(record);res.status(201).json({decision:record,receipt:record.receipt,aiAdvisoryOnly:true,humanAuthorizationRequired:true});}catch(e){res.status(400).json({error:e.message});}});
app.get('/api/ai/receipts/:id', requireSession(), (req,res)=>{const receipt=sprint24Runtime.receipts.find(x=>x.receiptId===req.params.id&&x.tenant===req.accessSession.tenant);if(!receipt)return res.status(404).json({error:'Receipt not found.'});res.json({receipt});});
app.patch('/api/ai/advisories/:id/status', requireSession(['hr','executive']), (req,res)=>res.status(409).json({error:'Advisory status changes require a recorded human decision; direct status mutation is blocked.'}));
app.post('/api/ai/advisories/:id/dismiss', requireSession(['hr','executive']), (req,res)=>res.status(409).json({error:'Use Close Without Action through the human decision endpoint with rationale.'}));
app.post('/api/ai/advisories/:id/escalate', requireSession(['manager','hr','executive']), (req,res)=>{req.body={...(req.body||{}),decision:'ESCALATE_FOR_REVIEW'};const advisory=sprint24Runtime.advisories.find(x=>x.advisoryId===req.params.id);if(!advisory||!sprint24CanView(req,advisory))return res.status(404).json({error:'Advisory not found.'});return res.status(409).json({error:'Use the human decision endpoint with an explicit rationale and workflow reference; automatic escalation is blocked.'});});
