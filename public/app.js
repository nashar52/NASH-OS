...
function appShell(kind, eyebrow, title, subtitle, body, actions='') {
  $('operationSurface').classList.remove('hidden');
...
async function employeeAttendanceApp() {
  ...
  appShell('attendance','TIME & ATTENDANCE','Attendance & Timesheet',`Daily presence, shifts, exceptions, overtime, and correction requests for ${name}.`, `
    ...
    <div class="domain-two-column">
      <section class="domain-panel"><div class="panel-title"><div><p class="eyebrow">MONTH VIEW</p><h3>July attendance calendar</h3></div><button class="secondary-btn small">Export timesheet</button></div>
      ...
      <section class="domain-panel"><p class="eyebrow">EXCEPTIONS</p><h3>Needs attention</h3><div class="domain-list">
        <article><span>Late arrival · 8 Jul</span><strong>12 min</strong><button class="text-btn">Request correction</button></article>
        <article><span>Overtime · 16 Jul</span><strong>2.0h</strong><button class="text-btn">View approval</button></article>
        <article><span>Missing checkout · 20 Jul</span><strong>Open</strong><button class="text-btn">Add evidence</button></article>
      </div></section>
    </div>`);
...
async function employeeTasksApp() {
  ...
  appShell('tasks','WORK MANAGEMENT','Tasks & Evidence','Manage assigned work, deadlines, evidence files, and completion receipts.',`<button class="primary-btn" data-new-evidence>Submit Evidence</button>`,`
    ...
    <section class="domain-panel"><div class="panel-title"><div><p class="eyebrow">MY QUEUE</p><h3>Assigned work</h3></div><div class="table-tools"><input id="taskSearch" placeholder="Search tasks"><button class="secondary-btn small">Filter</button></div></div>
...
async function employeeFileApp() {
  ...
  appShell('employee-file','EMPLOYEE RECORD','My Employee File','Personal, employment, qualification, and document records in one controlled profile.',`<button class="primary-btn" data-upload-file>Upload Document</button>`,`
    ...
    <nav class="record-tabs"><button class="active">Overview</button><button>Employment</button><button>Qualifications</button><button>Documents</button><button>Emergency</button><button>History</button></nav>
    <div class="domain-two-column">
...
async function employeePerformanceApp() {
  ...
  appShell('performance','PERFORMANCE','My Performance','Objectives, competencies, feedback, and development outcomes for the current review cycle.',`<button class="primary-btn">Open Self Review</button>`,`
...
async function employeeRightsApp() {
  await ensureEmployeeContext();
  appShell('rights','SELF SERVICE','Rights & Reports','Personal entitlements, requests, letters, payslips, and downloadable reports.',`<button class="primary-btn">Create Request</button>`,`
    <div class="service-grid"><button><span>Leave balance</span><strong>24 days</strong><small>View and request leave</small></button><button><span>Payslips</span><strong>12</strong><small>Download salary statements</small></button><button><span>Employment letter</span><strong>Generate</strong><small>Arabic or English</small></button><button><span>End-of-service estimate</span><strong>Preview</strong><small>Non-binding calculation</small></button><button><span>Benefits</span><strong>Active</strong><small>Medical and allowances</small></button><button><span>My requests</span><strong>3 open</strong><small>Track approval status</small></button></div>
    <section class="domain-panel"><p class="eyebrow">REQUEST HISTORY</p><h3>Recent employee services</h3><div class="table-wrap"><table class="domain-table"><thead><tr><th>Request</th><th>Submitted</th><th>Owner</th><th>Status</th><th></th></tr></thead><tbody><tr><td>Employment letter</td><td>20 Jul 2026</td><td>HR Services</td><td><span class="status-pill">Completed</span></td><td><button class="text-btn">Download</button></td></tr><tr><td>Leave request</td><td>18 Jul 2026</td><td>Manager</td><td><span class="status-pill">Pending</span></td><td><button class="text-btn">Track</button></td></tr><tr><td>Bank details update</td><td>10 Jul 2026</td><td>Payroll</td><td><span class="status-pill">Verified</span></td><td><button class="text-btn">View</button></td></tr></tbody></table></div></section>`);
}
...
async function hrDomainApp(domain) {
  const configs={
    ...
    learning:['LEARNING & DEVELOPMENT','Learning Center','Skill gaps, learning paths, nominations, completions, and impact evaluation.',['Skill gaps','42','Active courses','18','Nominations','67','Completion','81%'],['Employee','Skill gap','Learning path','Due','Progress','Status']],
  ...
  appShell(`hr-${domain}`,c[0],c[1],c[2],`<button class="primary-btn">Create ${domain==='ai'?'Decision Packet':'Case'}</button>`,`<div class="task-metrics">${metrics.join('')}</div><section class="domain-panel"><div class="panel-title"><div><p class="eyebrow">OPERATIONS QUEUE</p><h3>${c[1]} queue</h3></div><div class="table-tools"><input placeholder="Search"><button class="secondary-btn small">Filter</button><button class="secondary-btn small">Export</button></div></div><div class="table-wrap"><table class="domain-table"><thead><tr>${c[4].map(h=>`<th>${h}</th>`).join('')}</tr></thead><tbody>${rows}</tbody></table></div></section>`);
}
...
async function runCommand(command) {
  try {
    const map = {
      ...
      hr_performance: () => hrDomainApp('performance'),
      hr_training: () => hrDomainApp('learning'),
      hr_compensation: () => hrDomainApp('compensation'),
