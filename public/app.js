...
async function governmentComplianceWorkspace() {
  if (!state.employees.length) state.employees = (await api('/api/employees')).employees || [];
  const data = await api('/api/government/dashboard');
  const metrics = data.metrics || {}; const cases = data.cases || []; const tasks = data.tasks || []; const approvals = data.approvals || [];
  const employeeOptions = state.employees.map((employee) => `<option value="${esc(employee.id)}">${esc(employee.employeeCode)} · ${esc(employee.displayName)}</option>`).join('');
  const rows = cases.length ? cases.map((item) => `<tr><td><strong>${esc(item.employeeName)}</strong><small>${esc(item.service)} · ${esc(item.reference)}</small></td><td>${esc(item.expiryDate || '—')}</td><td>${esc(item.owner)}</td><td><span class="status-pill">${esc(item.status)}</span></td><td><button class="secondary-btn small" data-government-case="${esc(item.id)}">Open</button></td></tr>`).join('') : '<tr><td colspan="5" class="empty-state">No runtime compliance cases yet. Create a controlled case from a MySQL employee.</td></tr>';
  openOperation('Saudi Government Relations & Compliance', 'Qiwa, permits, Iqama, GOSI, Mudad/WPS, Nitaqat, labor-law readiness, fees, evidence, approvals, escalations, and SLA control. MySQL remains the source of truth; government submissions are not automated.', `
    <section class="hr-ops-shell"><div class="hr-ops-hero"><div><p class="eyebrow">SAUDI COMPLIANCE CONTROL TOWER</p><h2>Human-controlled government readiness</h2><p>Every case has accountable ownership, evidence and approval records. External Qiwa, GOSI, Mudad/WPS, Muqeem, and government transactions remain outside this platform.</p></div><div class="hr-source-card"><span>Source of truth</span><strong>MySQL employee master</strong><small>No schema or source-data mutation</small></div></div>
    <div class="hr-kpi-grid"><article><span>Active cases</span><strong>${esc(metrics.activeCases || 0)}</strong><small>Compliance register</small></article><article><span>Expiry ≤30 days</span><strong>${esc(metrics.expiringSoon || 0)}</strong><small>Contract / permit / Iqama evidence</small></article><article><span>Pending approvals</span><strong>${esc(metrics.pendingApprovals || 0)}</strong><small>Human decision required</small></article><article><span>SLA breaches</span><strong>${esc(metrics.breachedSla || 0)}</strong><small>Escalate to owner</small></article></div>
    <section class="domain-panel"><div class="panel-title"><div><p class="eyebrow">CREATE CASE</p><h3>Register a controlled compliance obligation</h3></div></div><form id="governmentCaseForm" class="form-grid"><label class="form-field"><span>Employee</span><select name="employeeId" required><option value="">Select MySQL employee</option>${employeeOptions}</select></label><label class="form-field"><span>Service</span><select name="service" required><option value="QIWA_CONTRACT">Qiwa contract</option><option value="WORK_PERMIT">Work permit</option><option value="IQAMA">Iqama</option><option value="GOSI">GOSI reconciliation</option><option value="MUDAD_WPS">Mudad / WPS readiness</option><option value="NITAQAT">Nitaqat / Saudization</option><option value="LABOR_LAW">Saudi labor law check</option><option value="GOVERNMENT_FEE">Government fee</option></select></label><label class="form-field"><span>Reference</span><input name="reference" required maxlength="120" placeholder="Contract, permit, or filing reference"></label><label class="form-field"><span>Expiry date</span><input name="expiryDate" type="date"></label><label class="form-field"><span>Compliance due date</span><input name="dueDate" type="date"></label><label class="form-field"><span>Accountable owner</span><input name="owner" required maxlength="120" placeholder="Government Relations owner"></label><label class="form-field"><span>Severity</span><select name="severity"><option>MEDIUM</option><option>LOW</option><option>HIGH</option><option>CRITICAL</option></select></label><label class="form-field"><span>Labor-law review note</span><input name="laborLawCheck" maxlength="300" value="Human legal review required"></label><button class="primary-btn" type="submit">Create compliance case</button></form></section>
    <section class="domain-panel"><div class="panel-title"><div><p class="eyebrow">COMPLIANCE REGISTER</p><h3>Cases, expiry alerts, and ownership</h3></div><button class="secondary-btn small" id="governmentExport">Export register</button></div><div class="table-wrap"><table class="domain-table"><thead><tr><th>Employee / service</th><th>Expiry</th><th>Owner</th><th>Status</th><th>Action</th></tr></thead><tbody>${rows}</tbody></table></div></section>
    <section class="domain-panel"><div class="panel-title"><div><p class="eyebrow">WORKFLOW STATUS</p><h3>Tasks, approvals, evidence, and fees</h3></div></div><div class="hr-risk-list"><div class="hr-risk-row"><span>Open remediation tasks</span><strong>${tasks.filter((item) => item.status !== 'COMPLETED').length}</strong><em class="watch">SLA tracked</em></div><div class="hr-risk-row"><span>Human approvals pending</span><strong>${approvals.filter((item) => item.status === 'PENDING').length}</strong><em class="watch">Decision required</em></div><div class="hr-risk-row"><span>Evidence ledger entries</span><strong>${(data.evidence || []).length}</strong><em class="good">Receipt-based</em></div><div class="hr-risk-row"><span>Government fee records</span><strong>${(data.fees || []).length}</strong><em class="good">SAR tracked</em></div></div></section></section>`);
  $('governmentCaseForm').onsubmit = async (event) => { event.preventDefault(); const out = await api('/api/government/cases', { method: 'POST', body: Object.fromEntries(new FormData(event.currentTarget)) }); addReceipt({ ...out.audit, actionType: 'GOVERNMENT_CASE_CREATED', target: out.item.reference, note: out.message }); toast(out.message); governmentComplianceWorkspace(); };
  $('governmentExport').onclick = () => downloadCsv(`saudi-compliance-register-${new Date().toISOString().slice(0, 10)}.csv`, [['Employee','Service','Reference','Expiry','Owner','Status'], ...cases.map((item) => [item.employeeName, item.service, item.reference, item.expiryDate || '', item.owner, item.status])]);
  document.querySelectorAll('[data-government-case]').forEach((button) => button.onclick = () => governmentCaseWorkspace(button.dataset.governmentCase));
}

async function governmentCaseWorkspace(caseId) {
  const data = await api('/api/government/dashboard'); const item = (data.cases || []).find((entry) => entry.id === caseId); if (!item) return governmentComplianceWorkspace();
  const relatedTasks = (data.tasks || []).filter((entry) => entry.caseId === item.id); const relatedApprovals = (data.approvals || []).filter((entry) => entry.caseId === item.id);
  openOperation(`Compliance Case · ${item.reference}`, 'Execute a documented, human-controlled compliance workflow. No button submits to a government platform or changes MySQL source records.', `<section class="hr-ops-shell"><div class="hr-ops-hero"><div><p class="eyebrow">${esc(item.service)}</p><h2>${esc(item.employeeName)}</h2><p>Owner: ${esc(item.owner)} · Expiry: ${esc(item.expiryDate || 'Not recorded')} · Status: ${esc(item.status)}</p></div><div class="hr-source-card"><span>Severity</span><strong>${esc(item.severity)}</strong><small>Human approval boundary</small></div></div><section class="domain-panel"><div class="panel-title"><div><p class="eyebrow">CASE ACTIONS</p><h3>Evidence, task, approval, fee, and escalation</h3></div></div><form id="governmentActionForm" class="form-grid"><label class="form-field"><span>Action</span><select name="action"><option value="CREATE_TASK">Create remediation task</option><option value="REQUEST_EVIDENCE">Request evidence</option><option value="REQUEST_APPROVAL">Request human approval</option><option value="RECORD_FEE">Record government fee</option><option value="ESCALATE">Escalate SLA / risk</option><option value="PLACE_PAYROLL_HOLD">Place payroll hold</option><option value="RELEASE_PAYROLL_HOLD">Release payroll hold</option><option value="APPROVE">Record human approval</option><option value="RETURN">Return for correction</option><option value="REJECT">Reject case</option></select></label><label class="form-field"><span>Action note / evidence reference</span><input name="note" maxlength="500" placeholder="Required except hold/release"></label><label class="form-field"><span>Title</span><input name="title" maxlength="160" placeholder="Task or evidence title"></label><label class="form-field"><span>Owner / approver</span><input name="owner" maxlength="120" placeholder="Task owner"><input name="approver" maxlength="120" placeholder="Approver"></label><label class="form-field"><span>Due date</span><input name="dueDate" type="date"></label><label class="form-field"><span>Evidence / fee reference</span><input name="reference" maxlength="240" placeholder="Receipt or invoice reference"></label><label class="form-field"><span>Expiry date</span><input name="expiryDate" type="date"></label><label class="form-field"><span>Fee amount (SAR)</span><input name="amount" type="number" min="0" step="0.01"></label><label class="form-field"><span>SLA hours</span><input name="slaHours" type="number" min="1" max="720" value="48"></label><button class="primary-btn" type="submit">Record action</button></form></section><section class="domain-panel"><div class="panel-title"><div><p class="eyebrow">CASE LEDGER</p><h3>Linked workflow artifacts</h3></div></div><div class="hr-risk-list"><div class="hr-risk-row"><span>Tasks</span><strong>${relatedTasks.length}</strong><em class="watch">${esc(relatedTasks.map((entry) => entry.status).join(', ') || 'None')}</em></div><div class="hr-risk-row"><span>Approvals</span><strong>${relatedApprovals.length}</strong><em class="watch">${esc(relatedApprovals.map((entry) => entry.status).join(', ') || 'None')}</em></div><div class="hr-risk-row"><span>Payroll hold</span><strong>${item.payrollHold ? 'Active' : 'Not active'}</strong><em class="${item.payrollHold ? 'watch' : 'good'}">${item.payrollHold ? 'Release after evidence' : 'No hold'}</em></div></div><button class="secondary-btn" id="governmentBack">Back to compliance register</button></section></section>`);
  $('governmentBack').onclick = governmentComplianceWorkspace;
  $('governmentActionForm').onsubmit = async (event) => { event.preventDefault(); const values = Object.fromEntries(new FormData(event.currentTarget)); const out = await api(`/api/government/cases/${encodeURIComponent(item.id)}/action`, { method: 'POST', body: values }); addReceipt({ ...out.audit, actionType: `GOVERNMENT_${values.action}`, target: item.reference, note: out.message }); toast(out.message); governmentCaseWorkspace(item.id); };
}
...
async function recruitmentWorkspace() {
  const data = await api('/api/recruitment/dashboard'); const m = data.metrics || {}; const requisitions = data.records.requisitions || []; const candidates = data.records.candidates || []; const interviews = data.records.interviews || []; const offers = data.records.offers || [];
  const reqOptions = requisitions.map(x => `<option value="${esc(x.id)}">${esc(x.title)} · ${esc(x.approvalStatus)}</option>`).join(''); const candidateOptions = candidates.map(x => `<option value="${esc(x.id)}">${esc(x.fullName)} · ${esc(x.stage)}</option>`).join('');
  const pipeline = (data.pipeline || []).map(x => `<div class="detail-card"><span>${esc(x.stage)}</span><strong>${esc(x.count)}</strong></div>`).join('');
  const actionButton = (action, label) => `<button class="secondary-btn small" data-recruitment-action="${action}">${label}</button>`;
  openOperation('Recruitment & ATS', 'Controlled recruiting operations: requisition approval, postings, career portal, candidates, interviews, offers, background checks, onboarding handoff, and source-labelled analytics. MySQL remains the source of truth.', `
    <section class="hr-ops-shell"><div class="hr-ops-hero"><div><p class="eyebrow">ENTERPRISE RECRUITMENT</p><h2>Hiring control center</h2><p>AI matching is advisory only. Position and hiring approvals always remain human-controlled.</p></div><div class="hr-source-card"><span>Source of truth</span><strong>MySQL</strong><small>No schema or source-row changes</small></div></div>
    <div class="detail-grid"><div><span>Open requisitions</span><strong>${esc(m.openRequisitions || 0)}</strong></div><div><span>Candidate profiles</span><strong>${esc(m.candidates || 0)}</strong></div><div><span>Time to hire</span><strong>${esc(m.timeToHireDays || 0)} days</strong></div><div><span>Cost per hire</span><strong>${esc(m.costPerHire || 0)}</strong></div></div>
    <section class="domain-panel"><div class="panel-title"><div><p class="eyebrow">PIPELINE ANALYTICS</p><h3>Recruitment funnel</h3></div></div><div class="detail-grid">${pipeline}</div></section>
    <section class="domain-panel"><div class="panel-title"><div><p class="eyebrow">CONTROLLED ACTIONS</p><h3>Recruitment workflows</h3></div></div><div class="action-grid">${actionButton('requisition','New requisition')}${actionButton('posting','Post a job')}${actionButton('candidate','Add candidate / parse resume')}${actionButton('interview','Schedule interview')}${actionButton('scorecard','Submit scorecard')}${actionButton('assessment','Record assessment')}${actionButton('offer','Create offer')}${actionButton('background','Request background check')}</div></section>
    <section class="domain-panel"><div class="panel-title"><div><p class="eyebrow">ACTIVE REGISTER</p><h3>Approval and hiring work</h3></div></div><div class="table-wrap"><table class="domain-table"><thead><tr><th>Record</th><th>Status / stage</th><th>Controlled action</th></tr></thead><tbody>${[...requisitions.map(x => `<tr><td><strong>${esc(x.title)}</strong><small>${esc(x.department)}</small></td><td>${esc(x.approvalStatus)}</td><td>${x.approvalStatus === 'PENDING_POSITION_APPROVAL' ? `<button class="secondary-btn small" data-req-approve="${esc(x.id)}">Approve position</button>` : actionButton('posting', 'Publish')}</td></tr>`), ...candidates.map(x => `<tr><td><strong>${esc(x.fullName)}</strong><small>${esc(x.email)}</small></td><td>${esc(x.stage)}${x.matchScore !== null ? ` · match ${esc(x.matchScore)}%` : ''}</td><td><button class="secondary-btn small" data-candidate-match="${esc(x.id)}">AI match</button> <button class="secondary-btn small" data-candidate-stage="${esc(x.id)}">Advance</button></td></tr>`), ...offers.map(x => `<tr><td><strong>Offer · ${esc(x.id)}</strong><small>${esc(x.startDate)}</small></td><td>${esc(x.status)}</td><td>${x.status === 'PENDING_APPROVAL' ? `<button class="primary-btn small" data-offer-approve="${esc(x.id)}">Approve hire</button>` : '—'}</td></tr>`)].join('') || '<tr><td colspan="3" class="empty-state">No recruitment records yet. Start with a requisition.</td></tr>'}</tbody></table></div></section></section>`);
  document.querySelectorAll('[data-recruitment-action]').forEach(button => button.onclick = () => recruitmentForm(button.dataset.recruitmentAction, { reqOptions, candidateOptions, interviews }));
  document.querySelectorAll('[data-req-approve]').forEach(button => button.onclick = async () => { const out = await api(`/api/recruitment/requisitions/${button.dataset.reqApprove}/approval`, { method: 'POST', body: { decision: 'APPROVE', note: 'Approved in ATS workspace.' } }); addReceipt({ ...out.audit, actionType: 'POSITION_APPROVED', target: out.requisition.title, note: out.message }); toast(out.message); recruitmentWorkspace(); });
  document.querySelectorAll('[data-candidate-match]').forEach(button => button.onclick = async () => { const skills = window.prompt('Required skills (comma-separated):'); if (skills === null) return; const out = await api(`/api/recruitment/candidates/${button.dataset.candidateMatch}/match`, { method: 'POST', body: { requiredSkills: skills } }); addReceipt({ ...out.audit, actionType: 'AI_CANDIDATE_MATCH', target: out.candidate.fullName, note: out.candidate.matchExplanation }); toast(`Advisory match: ${out.candidate.matchScore}%`); recruitmentWorkspace(); });
  document.querySelectorAll('[data-candidate-stage]').forEach(button => button.onclick = async () => { const stage = window.prompt('Stage: SCREENING, INTERVIEW, ASSESSMENT, OFFER, HIRED, or REJECTED', 'SCREENING'); if (!stage) return; const out = await api(`/api/recruitment/candidates/${button.dataset.candidateStage}/stage`, { method: 'POST', body: { stage } }); addReceipt({ ...out.audit, actionType: 'CANDIDATE_STAGE_CHANGED', target: out.candidate.fullName, note: out.message }); toast(out.message); recruitmentWorkspace(); });
  document.querySelectorAll('[data-offer-approve]').forEach(button => button.onclick = async () => { const out = await api(`/api/recruitment/offers/${button.dataset.offerApprove}/approval`, { method: 'POST', body: { decision: 'APPROVE' } }); addReceipt({ ...out.audit, actionType: 'HIRING_APPROVED', target: out.offer.id, note: out.message }); toast(out.message); recruitmentWorkspace(); });
}

function recruitmentForm(action, context) {
  const forms = { requisition: ['New job requisition', '/api/recruitment/requisitions', '<label>Job title<input name="title" required></label><label>Department<input name="department" required></label><label>Location<input name="location"></label><label>Hiring manager<input name="hiringManager" required></label><label>Recruiting cost<input name="recruitingCost" type="number" min="0"></label>'], posting: ['Publish job to career portal', '/api/recruitment/postings', `<label>Approved requisition<select name="requisitionId" required><option value="">Select</option>${context.reqOptions}</select></label><label>Channel<select name="channel"><option>INTERNAL</option><option>EXTERNAL</option></select></label>`], candidate: ['Candidate profile and resume parsing', '/api/recruitment/candidates', `<label>Requisition<select name="requisitionId" required><option value="">Select</option>${context.reqOptions}</select></label><label>Candidate name<input name="fullName" required></label><label>Email<input name="email" type="email" required></label><label>Skills (comma-separated)<input name="skills"></label><label>Resume text<textarea name="resumeText" placeholder="Paste resume text to parse skills and profile evidence"></textarea></label>`], interview: ['Schedule interview', '/api/recruitment/interviews', `<label>Candidate<select name="candidateId" required><option value="">Select</option>${context.candidateOptions}</select></label><label>Interview date & time<input name="scheduledAt" type="datetime-local" required></label><label>Interviewer<input name="interviewer" required></label><label>Format<select name="format"><option>Video</option><option>On-site</option><option>Phone</option></select></label>`], scorecard: ['Interview scorecard', '/api/recruitment/scorecards', `<label>Interview<select name="interviewId" required><option value="">Select</option>${context.interviews.map(x => `<option value="${esc(x.id)}">${esc(x.id)} · ${esc(x.interviewer)}</option>`).join('')}</select></label><label>Score (1–5)<input name="score" type="number" min="1" max="5" required></label><label>Recommendation<textarea name="recommendation" required></textarea></label>`], assessment: ['Assessment management', '/api/recruitment/assessments', `<label>Candidate<select name="candidateId" required><option value="">Select</option>${context.candidateOptions}</select></label><label>Assessment name<input name="name" required></label><label>Result<textarea name="result" required></textarea></label>`], offer: ['Create offer', '/api/recruitment/offers', `<label>Candidate<select name="candidateId" required><option value="">Select</option>${context.candidateOptions}</select></label><label>Salary<input name="salary" type="number" min="0" required></label><label>Proposed start date<input name="startDate" type="date" required></label>`], background: ['Background check workflow', '/api/recruitment/background-checks', `<label>Candidate<select name="candidateId" required><option value="">Select</option>${context.candidateOptions}</select></label><label>Authorized provider<input name="provider" required></label>`] };
